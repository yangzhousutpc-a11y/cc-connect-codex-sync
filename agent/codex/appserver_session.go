...
type appServerRequestDelivery uint8

const (
	appServerRequestNotSent appServerRequestDelivery = iota
	appServerRequestRejected
	appServerRequestUnknown
	appServerRequestCompleted
)
...
func newAppServerSession(ctx context.Context, url, workDir, cliBin string, cliExtraArgs []string, model, effort, mode, resumeID, recoveryKey, baseURL, modelProvider string, extraEnv []string, codexHome string, systemPrompt string, appendPrompt string, desktopOrigins *desktopOriginTracker) (*appServerSession, error) {
	...
	if err := s.ensureThread(resumeID, recoveryKey); err != nil {
		_ = s.Close()
		return nil, err
	}
...
func (s *appServerSession) connect() error {
	useLocalDaemon := os.Getenv("CODEX_APP_SERVER_USE_LOCAL_DAEMON") == "1"
	args := []string{"app-server", "proxy"}
	transport := "shared-daemon-proxy"
	if !useLocalDaemon {
	...
	slog.Info("codex app-server session started", "transport", transport, "pid", cmd.Process.Pid, "work_dir", s.workDir)
...
func (s *appServerSession) ensureThread(resumeID, recoveryKey string) error {
	recoveryKey = strings.TrimSpace(recoveryKey)
	if resumeID == "" || resumeID == core.ContinueSession {
		recoveredID, err := s.findThreadByRecoveryKey(recoveryKey)
		if err != nil {
			return fmt.Errorf("codex app-server find recovery thread: %w", err)
		}
		if recoveredID != "" {
			resumeID = recoveredID
		}
	}
	if resumeID != "" && resumeID != core.ContinueSession {
		...
		err := s.request("thread/resume", params, &resp)
		if isArchivedThreadResumeError(err) {
			if unarchiveErr := s.request("thread/unarchive", map[string]any{"threadId": resumeID}, nil); unarchiveErr != nil {
				return fmt.Errorf("%w: codex app-server unarchive thread %s: %v", core.ErrSessionContextPreservationRequired, resumeID, unarchiveErr)
			}
			s.refreshThread(resumeID)
			resp = threadResumeResponse{}
			if retryErr := s.request("thread/resume", params, &resp); retryErr != nil {
				return fmt.Errorf("%w: codex app-server resume unarchived thread %s: %v", core.ErrSessionContextPreservationRequired, resumeID, retryErr)
			}
			err = nil
		}
		if err != nil {
			if recoveryKey == "" || !strings.Contains(err.Error(), "no rollout found for thread id") {
				return err
			}
			slog.Warn("codex app-server replacing unmaterialized recovery thread", "thread_id", resumeID)
		} else {
			if resp.Thread.ID == "" {
	...
	params := s.threadRequestParams()
	if recoveryKey != "" {
		params["threadSource"] = recoveryKey
	}
	...
	s.applyThreadRuntimeState(resp.Cwd, resp.Model, resp.ReasoningEffort)
	s.threadID.Store(resp.Thread.ID)
	s.threadStartedFresh = true
	slog.Info("codex app-server thread started", "thread_id", resp.Thread.ID)
	return nil
}

func isArchivedThreadResumeError(err error) bool {
	if err == nil {
		return false
	}
	message := strings.ToLower(strings.TrimSpace(err.Error()))
	return strings.Contains(message, "is archived") &&
		(strings.Contains(message, "session") || strings.Contains(message, "thread"))
}

func (s *appServerSession) findThreadByRecoveryKey(recoveryKey string) (string, error) {
	recoveryKey = strings.TrimSpace(recoveryKey)
	if recoveryKey == "" {
		return "", nil
	}
	wantDir := canonicalThreadWorkDir(s.workDir)
	cursor := ""
	...
	for {
		...
		if err := s.request("thread/list", params, &resp); err != nil {
			return "", err
		}
		for _, thread := range resp.Data {
			if thread.ThreadSource == recoveryKey && canonicalThreadWorkDir(thread.Cwd) == wantDir {
		...
		if _, duplicate := seenCursors[cursor]; duplicate {
			return "", fmt.Errorf("thread/list returned repeated cursor %q", cursor)
		}
		seenCursors[cursor] = struct{}{}
	}
}

func canonicalThreadWorkDir(dir string) string {
...
func (s *appServerSession) PrimeSession(name string) error {
	s.primeMu.Lock()
	...
	if !s.threadStartedFresh {
		var thread threadReadResponse
		if err := s.request("thread/read", map[string]any{
			"threadId":     threadID,
			"includeTurns": true,
		}, &thread); err != nil {
			return fmt.Errorf("codex app-server inspect visibility: %w", err)
		}
		if len(thread.Thread.Turns) > 0 {
			s.sessionPrimed = true
			s.refreshThread(threadID)
			return nil
		}
	}
	...
	name = strings.TrimSpace(name)
	prompt := "这是由飞书 /new 创建的新会话。不要调用工具或执行任何任务，只回复：会话已创建。"
	if name != "" {
		prompt = fmt.Sprintf("这是由飞书 /new 创建的新会话“%s”。不要调用工具或执行任何任务，只回复：会话已创建。", name)
	}
	if err := s.Send(prompt, nil, nil); err != nil {
		return fmt.Errorf("codex app-server start visibility turn: %w", err)
	}
	...
	for {
		...
		case event, ok := <-s.Events():
			if !ok {
				return fmt.Errorf("codex app-server visibility turn event stream closed")
			}
			switch event.Type {
			case core.EventResult:
				if event.Done {
					s.sessionPrimed = true
					s.refreshThread(s.CurrentSessionID())
					return nil
				}
			case core.EventError:
				return s.abortPrimeTurn(fmt.Errorf("codex app-server visibility turn failed: %w", event.Error))
			case core.EventPermissionRequest:
				return s.abortPrimeTurn(fmt.Errorf("codex app-server visibility turn unexpectedly requested permission"))
			}
		case <-ctx.Done():
			return ctx.Err()
		case <-timer.C:
			return s.abortPrimeTurn(fmt.Errorf("codex app-server visibility turn timed out"))
		}
	}
}

func (s *appServerSession) abortPrimeTurn(reason error) error {
	if err := s.CancelTurn(); err != nil {
		return errors.Join(reason, fmt.Errorf("interrupt visibility turn: %w", err))
	}
	...
	for {
		...
		case event, ok := <-s.Events():
			if !ok {
				return errors.Join(reason, fmt.Errorf("visibility turn event stream closed while interrupting"))
			}
			if event.Type == core.EventResult && event.Done {
				return reason
			}
		case <-s.ctx.Done():
			return errors.Join(reason, s.ctx.Err())
		case <-timer.C:
			return errors.Join(reason, fmt.Errorf("visibility turn did not stop after interrupt"))
		}
	}
}

func (s *appServerSession) refreshThread(threadID string) {
	if s.refreshThreadInUI != nil {
		s.refreshThreadInUI(threadID)
		return
	}
	refreshCodexAppThreadInUI(threadID)
}

func (s *appServerSession) ArchiveSession(threadID string) error {
	threadID = strings.TrimSpace(threadID)
	if threadID == "" {
		return nil
	}
	if err := s.request("thread/archive", map[string]any{"threadId": threadID}, nil); err != nil {
		return fmt.Errorf("codex app-server archive thread: %w", err)
	}
	notifyCodexAppThreadArchived(threadID)
	return nil
}

func refreshCodexAppThreadInUI(threadID string) {
	home, err := os.UserHomeDir()
	if err != nil {
		slog.Debug("codex app thread refresh skipped", "error", err)
		return
	}
	if err := refreshCodexAppThread(filepath.Join(home, ".codex", "ipc", "ipc.sock"), threadID); err != nil {
		slog.Debug("codex app thread refresh skipped", "thread_id", threadID, "error", err)
	}
}

func notifyCodexAppThreadArchived(threadID string) {
	home, err := os.UserHomeDir()
	if err != nil {
		return
	}
	conn, err := net.DialTimeout("unix", filepath.Join(home, ".codex", "ipc", "ipc.sock"), time.Second)
	if err != nil {
		return
	}
	defer conn.Close()
	_ = conn.SetWriteDeadline(time.Now().Add(time.Second))
	_ = writeCodexAppIPCBroadcast(conn, "thread-archived", 2, map[string]any{
		"hostId": "local", "conversationId": threadID,
	})
}

func refreshCodexAppThread(socketPath, threadID string) error {
	...
	if err := writeCodexAppIPCBroadcast(conn, "thread-archived", 2, params); err != nil {
		return err
	}
	return writeCodexAppIPCBroadcast(conn, "thread-unarchived", 1, params)
}

func writeCodexAppIPCBroadcast(conn net.Conn, method string, version int, params map[string]any) error {
	message, err := json.Marshal(map[string]any{
		"type":           "broadcast",
		"method":         method,
		"sourceClientId": "cc-connect-session-name-sync",
		"version":        version,
		"params":         params,
	})
	if err != nil {
...
func (s *appServerSession) Send(prompt string, images []core.ImageAttachment, files []core.FileAttachment) error {
	...
	var originTicket desktopOriginTicket
	if s.desktopOrigins != nil {
		originTicket = s.desktopOrigins.begin(threadID)
	}

	var resp turnStartResponse
	delivery, err := s.requestWithDelivery("turn/start", params, &resp, appServerRequestTimeout)
	if err != nil {
		if s.desktopOrigins != nil && (delivery == appServerRequestNotSent || delivery == appServerRequestRejected) {
			s.desktopOrigins.cancel(originTicket)
		}
	...
	if s.desktopOrigins != nil {
		s.desktopOrigins.complete(originTicket, resp.Turn.ID)
	}
...
func (s *appServerSession) CancelTurn() error {
	...
	if err := s.requestWithTimeout("turn/interrupt", map[string]any{
		"threadId": threadID,
		"turnId":   turnID,
	...
	case "turn/started":
		...
		if err := json.Unmarshal(paramsRaw, &notif); err == nil {
			if s.desktopOrigins != nil {
				...
				s.desktopOrigins.reconcile(threadID, notif.Turn.ID)
			}
			s.stateMu.Lock()
...
func (s *appServerSession) requestWithTimeout(method string, params any, out any, timeout time.Duration) error {
	_, err := s.requestWithDelivery(method, params, out, timeout)
	return err
}

func (s *appServerSession) requestWithDelivery(method string, params any, out any, timeout time.Duration) (appServerRequestDelivery, error) {
	id := s.nextID.Add(1)
	...
	written, err := s.writeJSONWithDelivery(payload)
	if err != nil {
		s.pendingMu.Lock()
		delete(s.pending, id)
		s.pendingMu.Unlock()
		if written {
			return appServerRequestUnknown, err
		}
		return appServerRequestNotSent, err
	}

	select {
	case resp := <-ch:
		if resp.Error != nil {
			return appServerRequestRejected, fmt.Errorf("%s", strings.TrimSpace(resp.Error.Message))
		}
		if out != nil {
			if err := json.Unmarshal(resp.Result, out); err != nil {
				return appServerRequestUnknown, fmt.Errorf("decode %s response: %w", method, err)
			}
		}
		return appServerRequestCompleted, nil
	case <-s.ctx.Done():
		return appServerRequestUnknown, s.ctx.Err()
	case <-time.After(timeout):
		s.pendingMu.Lock()
		delete(s.pending, id)
		s.pendingMu.Unlock()
		return appServerRequestUnknown, fmt.Errorf("%s timed out", method)
	}
}
...
func (s *appServerSession) writeJSON(v any) error {
	_, err := s.writeJSONWithDelivery(v)
	return err
}

func (s *appServerSession) writeJSONWithDelivery(v any) (bool, error) {
	b, err := json.Marshal(v)
	if err != nil {
	...
	if err != nil {
		return n > 0, fmt.Errorf("codex app-server write: %w", err)
	}
	if n != len(payload) {
		return n > 0, fmt.Errorf("codex app-server write: %w", io.ErrShortWrite)
	}
	return true, nil
}
