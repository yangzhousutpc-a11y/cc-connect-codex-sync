...
func TestGetModelAndReasoningEffort_FromRuntimeConfigWhenUnset(t *testing.T) {
	oldRuntimeConfigTimeout := codexRuntimeConfigTimeout
	codexRuntimeConfigTimeout = 10 * time.Second
	t.Cleanup(func() {
		codexRuntimeConfigTimeout = oldRuntimeConfigTimeout
	})
...
`
	...
	fakeCodex := fakeCodexExecutablePath(binDir)
	cs, err := newCodexSession(context.Background(), fakeCodex, nil, workDir, "", "", "", "", "", nil, "", "", "")
	if err != nil {
		t.Fatalf("newCodexSession: %v", err)
	}
	defer cs.Close()
...
`
	...
	cs, err := newCodexSession(context.Background(), fakeCodexExecutablePath(binDir), nil, workDir, "", "", "", "", "", nil, "", "", "")
	if err != nil {
		t.Fatalf("newCodexSession: %v", err)
	}
	defer cs.Close()
...
`
	...
	cs, err := newCodexSession(context.Background(), fakeCodexExecutablePath(binDir), nil, workDir, "", "", "", "thread-123", "", nil, "", "", "")
	if err != nil {
		t.Fatalf("newCodexSession: %v", err)
	}
	defer cs.Close()
...
`
	...
	cs, err := newCodexSession(context.Background(), fakeCodexExecutablePath(binDir), nil, workDir, "", "", "", "thread-stdin", "", nil, "", "", "")
	if err != nil {
		t.Fatalf("newCodexSession: %v", err)
	}
	defer cs.Close()
...
`
	...
	cs, err := newCodexSession(context.Background(), fakeCodexExecutablePath(binDir), nil, workDir, "", "", "", "", "", nil, "", "You are Linear Reporter.", "Always invoke linear-bug-intake.")
	if err != nil {
		t.Fatalf("newCodexSession: %v", err)
	}
...
func fakeCodexExecutablePath(dir string) string {
	if runtime.GOOS == "windows" {
