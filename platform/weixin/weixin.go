...
// SupportsInPlaceConversationFork reports that Weixin can fork a logical
// session without creating a separate Weixin conversation.
func (p *Platform) SupportsInPlaceConversationFork() bool { return true }
...
func (p *Platform) pollLoop(ctx context.Context) {
	...
	for {
		...
		admissions := make([]<-chan struct{}, 0, len(resp.Msgs))
		for i := range resp.Msgs {
			admissions = append(admissions, p.dispatchInboundOrdered(ctx, &resp.Msgs[i], h))
		}
...
// dispatchInboundOrdered preserves the server-provided order for one peer up
// to core admission. It deliberately does not wait for the handler's complete
// turn: once core acknowledges admission, the next same-peer message may enter
// and join core's transition/busy queue. Different peers use independent
// admission chains.
func (p *Platform) dispatchInboundOrdered(ctx context.Context, m *weixinMessage, h core.MessageHandler) <-chan struct{} {
	done := make(chan struct{})
	peer := ""
	if m != nil {
		peer = strings.TrimSpace(m.FromUserID)
	}
	admitted := make(chan struct{})

	p.inboundOrderMu.Lock()
	if p.inboundAdmissions == nil {
		p.inboundAdmissions = make(map[string]chan struct{})
	}
	previous := p.inboundAdmissions[peer]
	p.inboundAdmissions[peer] = admitted
	p.inboundOrderMu.Unlock()

	go func() {
		defer close(done)
		defer func() {
			p.inboundOrderMu.Lock()
			if p.inboundAdmissions[peer] == admitted {
				delete(p.inboundAdmissions, peer)
			}
			p.inboundOrderMu.Unlock()
		}()
		...
		p.dispatchInbound(ctx, m, func(platform core.Platform, msg *core.Message) {
			admission := make(chan struct{})
			msg.DispatchAdmission = admission
...
func (p *Platform) dispatchInbound(ctx context.Context, m *weixinMessage, h core.MessageHandler) {
	...
	rc := &replyContext{peerUserID: from, contextToken: strings.TrimSpace(m.ContextToken), sessionKey: sessionKey}
...
// ReplySessionKey returns the complete logical session key carried by a reply context.
func (p *Platform) ReplySessionKey(replyCtx any) string {
...
// FormatSessionReply labels outbound text with the active logical session name.
func (p *Platform) FormatSessionReply(sessionName, content string) string {
	if strings.TrimSpace(content) == "" {
		return content
	}
	prefix := sessionReplyPrefix(sessionName)
...
func sessionReplyPrefix(sessionName string) string {
	name := strings.TrimSpace(sessionName)
	name = strings.TrimSpace(strings.TrimPrefix(name, "💬"))
	name = strings.TrimSpace(strings.TrimPrefix(name, "[微信-Codex]"))
	name = strings.TrimSpace(strings.TrimPrefix(name, "[Codex]"))
	...
	runes := []rune(name)
	if len(runes) > maxWeixinSessionLabelRunes {
		name = string(runes[:maxWeixinSessionLabelRunes])
	}
	return "[" + name + "] "
}
...
func (p *Platform) sendChunks(ctx context.Context, replyCtx any, content string) error {
	...
	noticePrefix, chunks := splitSessionReplyChunks(content, maxWeixinChunk)
	total := len(chunks)
	for i, chunk := range chunks {
		...
		if err != nil {
			...
			notice := noticePrefix + "⚠️ 消息发送不完整，请在终端查看完整结果。"
...
func splitSessionReplyChunks(content string, maxRunes int) (string, []string) {
	if !strings.HasPrefix(content, "[") {
		return "", splitUTF8(content, maxRunes)
	}
	end := strings.Index(content, "] ")
	if end <= 1 {
		return "", splitUTF8(content, maxRunes)
	}
	prefix := sessionReplyPrefix(content[1:end])
	if !strings.HasPrefix(content, prefix) {
		return "", splitUTF8(content, maxRunes)
	}
	bodyLimit := maxRunes - utf8.RuneCountInString(prefix)
	if bodyLimit <= 0 {
		return "", splitUTF8(content, maxRunes)
	}
	bodyChunks := splitUTF8(content[len(prefix):], bodyLimit)
	chunks := make([]string, len(bodyChunks))
	for i, bodyChunk := range bodyChunks {
		chunks[i] = prefix + bodyChunk
	}
...
func (p *Platform) ReconstructReplyCtx(sessionKey string) (any, error) {
	...
	return &replyContext{peerUserID: peer, contextToken: tok, sessionKey: sessionKey}, nil
}

// ExternalConversationRelayEnabled allows Codex App transcript events to be
// relayed back to the active Weixin logical session.
