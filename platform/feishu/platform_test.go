...
func TestCmdAction_WithoutAfterClick_DispatchesAndReturnsNil(t *testing.T) {
	...
	resp, err := ip.onCardAction(&callback.CardActionTriggerEvent{
		EventV2Base: &larkevent.EventV2Base{Header: &larkevent.EventHeader{EventID: "evt_cmd_1"}},
		Event: &callback.CardActionTriggerRequest{
	...
	case msg := <-msgCh:
		...
		if msg.MessageID != "evt_cmd_1" {
			t.Fatalf("dispatched message ID = %q, want card event ID", msg.MessageID)
		}
	case <-time.After(2 * time.Second):
