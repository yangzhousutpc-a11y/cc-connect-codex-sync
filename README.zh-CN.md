...
- 文字消息和结构化图片会自动双向同步。
- 企业微信入站文件受支持，可以从企业微信群进入对应的 Codex 会话。
- 飞书、个人微信和企业微信统一遵守文件外发规则：普通文件不会仅因出现在 Codex App 的 Files block 中就自动发送。
- 在 Codex App 附加文件后，请明确说“把这个文件发送到当前群”。Agent 会使用 `cc-connect send --file` 将文件发送到当前会话对应的平台群聊或会话。
- 发送成功后只确认结果，不要在确认消息中展示本机路径。

这项限制只阻止未经明确指示的文件外发，不代表文件无法发送。
