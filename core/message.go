...
type Message struct {
	...
	// inPlaceTransition marks messages currently owned by the in-place
	// transition dispatcher. It is intentionally internal: platforms must not
	// persist or populate transition runtime state.
	inPlaceTransition        *inPlaceTransition
	inPlaceCompletion        *inPlaceMessageCompletion
	inPlaceControlTransition *inPlaceTransition
	inPlaceControlKind       inPlaceActiveTurnControl

	// DispatchAdmission is an optional platform-to-engine ordering barrier.
	// The engine closes it once the message has entered core dispatch ordering;
	// platforms may then admit the next message without waiting for this turn.
	DispatchAdmission chan struct{}
}

func (m *Message) acknowledgeDispatchAdmission() {
	if m == nil || m.DispatchAdmission == nil {
		return
	}
	select {
	case <-m.DispatchAdmission:
		return
	default:
		close(m.DispatchAdmission)
	}
}
