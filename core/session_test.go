...
func TestSessionPendingArchivePersists(t *testing.T) {
	storePath := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(storePath)
	s := sm.NewSession("user1", "new")
	s.SetPendingArchiveAgentSessionID("thread-old")
	sm.Save()

	reloaded := NewSessionManager(storePath).GetOrCreateActive("user1")
	if got := reloaded.GetPendingArchiveAgentSessionID(); got != "thread-old" {
		t.Fatalf("pending archive ID = %q, want %q", got, "thread-old")
	}
}

func TestAgentSessionRoutesOnlyIncludesActiveSessionCurrentID(t *testing.T) {
	sm := NewSessionManager("")
	inactive := sm.NewSession("feishu:chat-1:user-1", "inactive")
	inactive.SetAgentSessionID("thread-past", "codex")
	inactive.SetAgentSessionID("thread-inactive", "codex")
	active := sm.NewSession("feishu:chat-1:user-1", "active")
	active.SetAgentSessionID("thread-active", "codex")

	routes := sm.AgentSessionRoutes()
	if got := routes["thread-active"]; got != "feishu:chat-1:user-1" {
		t.Fatalf("active route = %q, want feishu:chat-1:user-1", got)
	}
	for _, threadID := range []string{"thread-past", "thread-inactive"} {
		if got := routes[threadID]; got != "" {
			t.Fatalf("inactive route[%q] = %q, want empty", threadID, got)
		}
	}
}

func TestAgentSessionRoutesFailsClosedForDuplicateActiveAgentSessionID(t *testing.T) {
	sm := NewSessionManager("")
	a := sm.NewSession("feishu:chat-a:user-1", "A")
	a.SetAgentSessionID("thread-duplicate", "codex")
	b := sm.NewSession("feishu:chat-b:user-1", "B")
	b.SetAgentSessionID("thread-duplicate", "codex")

	if got := sm.AgentSessionRoutes()["thread-duplicate"]; got != "" {
		t.Fatalf("duplicate active route = %q, want fail-closed empty route", got)
	}
}

func TestSwitchToAgentSessionRejectsThreadOwnedByAnotherSessionKey(t *testing.T) {
	sm := NewSessionManager("")
	a := sm.NewSession("feishu:chat-a:user-1", "A")
	...
	before := sm.ActiveSessionID("feishu:chat-a:user-1")
	claimed := sm.SwitchToAgentSession("feishu:chat-a:user-1", "thread-b", "codex", "B task")
	if claimed != nil {
		t.Fatalf("cross-group claim returned session %q, want nil", claimed.ID)
	}
	if got := sm.ActiveSessionID("feishu:chat-a:user-1"); got != before {
		t.Fatalf("A active session changed from %q to %q", before, got)
	}
	if got := sm.AgentSessionRoutes()["thread-b"]; got != "feishu:chat-b:user-1" {
		t.Fatalf("thread-b route = %q, want B group", got)
	}
}
...
func TestSessionManagerActivatePreparedAndBackPersist(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(path)
	a := sm.GetOrCreateActive("user1")
	b := sm.NewSideSession("user1", "prepared")

	if err := sm.ActivatePreparedSession("user1", b.ID, a.ID); err != nil {
		t.Fatalf("ActivatePreparedSession: %v", err)
	}
	if got := sm.ActiveSessionID("user1"); got != b.ID {
		t.Fatalf("active = %q, want %q", got, b.ID)
	}

	reloaded := NewSessionManager(path)
	back, err := reloaded.BackSession("user1")
	if err != nil {
		t.Fatalf("BackSession after reload: %v", err)
	}
	if back.ID != a.ID {
		t.Fatalf("back session = %q, want %q", back.ID, a.ID)
	}
}

func TestSessionManagerEnsurePreparedSideSessionScopesAndPersistsMarker(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	const key = "weixin:dm:peer"
	sm := NewSessionManager(path)
	legacy := sm.NewSideSession(key, "legacy candidate")
	first, err := sm.EnsurePreparedSideSession(key, "B", "op-1")
	if err != nil {
		t.Fatal(err)
	}
	if first.ID == legacy.ID {
		t.Fatal("legacy unmarked side session was reused")
	}

	reloaded := NewSessionManager(path)
	same, err := reloaded.EnsurePreparedSideSession(key, "ignored rename", "op-1")
	if err != nil {
		t.Fatal(err)
	}
	if same.ID != first.ID || same.GetName() != "B" {
		t.Fatalf("reloaded candidate = %#v, want original %q named B", same, first.ID)
	}
	otherOperation, err := reloaded.EnsurePreparedSideSession(key, "C", "op-2")
	if err != nil {
		t.Fatal(err)
	}
	if otherOperation.ID == first.ID {
		t.Fatal("different /new operation reused op-1 candidate")
	}
	otherUser, err := reloaded.EnsurePreparedSideSession("weixin:dm:other", "other", "op-1")
	if err != nil {
		t.Fatal(err)
	}
	if otherUser.ID == first.ID {
		t.Fatal("same marker in another external chat reused peer candidate")
	}

	workspaceStore := filepath.Join(t.TempDir(), "workspace-sessions.json")
	workspaceSessions := NewSessionManager(workspaceStore)
	workspaceCandidate, err := workspaceSessions.EnsurePreparedSideSession(key, "workspace B", "op-1")
	if err != nil {
		t.Fatal(err)
	}
	if workspaceCandidate.GetName() != "workspace B" || len(workspaceSessions.ListSessions(key)) != 1 {
		t.Fatalf("workspace candidate = %#v, want independently created candidate", workspaceCandidate)
	}
}

func TestSessionManagerActivatePreparedRejectsChangedSource(t *testing.T) {
	sm := NewSessionManager("")
	a := sm.GetOrCreateActive("user1")
	b := sm.NewSideSession("user1", "prepared")
	c := sm.NewSession("user1", "new active")

	err := sm.ActivatePreparedSession("user1", b.ID, a.ID)
	if err != ErrActiveSessionChanged {
		t.Fatalf("ActivatePreparedSession error = %v, want %v", err, ErrActiveSessionChanged)
	}
	if got := sm.ActiveSessionID("user1"); got != c.ID {
		t.Fatalf("active = %q, want %q", got, c.ID)
	}
}

func TestSessionManagerDeletePrunesBackStack(t *testing.T) {
	sm := NewSessionManager("")
	a := sm.GetOrCreateActive("user1")
	b := sm.NewSideSession("user1", "prepared")
	if err := sm.ActivatePreparedSession("user1", b.ID, a.ID); err != nil {
	...
	if !sm.DeleteByID(a.ID) {
		t.Fatal("DeleteByID returned false")
	}

	_, err := sm.BackSession("user1")
	if err != ErrNoPreviousSession {
		t.Fatalf("BackSession error = %v, want %v", err, ErrNoPreviousSession)
	}
}

func TestSwitchToAgentSessionWithBackRecordsPreviousActive(t *testing.T) {
	sm := NewSessionManager("")
	userKey := "weixin:user1"
	a := sm.GetOrCreateActive(userKey)
	a.SetAgentInfo("agent-A", "codex", "A")

	b, err := sm.SwitchToAgentSessionWithBack(userKey, "agent-B", "codex", "B")
	if err != nil {
		t.Fatalf("SwitchToAgentSessionWithBack: %v", err)
	}
	if b == nil {
		t.Fatal("SwitchToAgentSessionWithBack returned nil")
	}
	if got := sm.ActiveSessionID(userKey); got != b.ID {
		t.Fatalf("active = %q, want %q", got, b.ID)
	}
	back, err := sm.BackSession(userKey)
	if err != nil {
		t.Fatalf("BackSession: %v", err)
	}
	if back.ID != a.ID {
		t.Fatalf("back session = %q, want %q", back.ID, a.ID)
	}
...
func TestSessionManagerActivatePreparedRollsBackOnSaveFailure(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(path)
	a := sm.GetOrCreateActive("user1")
	b := sm.NewSideSession("user1", "prepared")
	sm.storePath = t.TempDir()

	if err := sm.ActivatePreparedSession("user1", b.ID, a.ID); err == nil {
		t.Fatal("ActivatePreparedSession succeeded despite save failure")
	}
	if got := sm.ActiveSessionID("user1"); got != a.ID {
		t.Fatalf("active = %q, want %q", got, a.ID)
	}
	if len(sm.backStack["user1"]) != 0 {
		t.Fatalf("back stack = %v, want empty", sm.backStack["user1"])
	}
	if len(sm.sessions) != 2 || len(sm.userSessions["user1"]) != 2 || sm.counter != 2 {
		t.Fatalf("state changed after failed activation: sessions=%d userSessions=%d counter=%d", len(sm.sessions), len(sm.userSessions["user1"]), sm.counter)
	}
}

func TestSessionManagerBackRollsBackOnSaveFailure(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(path)
	a := sm.GetOrCreateActive("user1")
	b := sm.NewSideSession("user1", "prepared")
	...
	if _, err := sm.BackSession("user1"); err == nil {
		t.Fatal("BackSession succeeded despite save failure")
	}
	if got := sm.ActiveSessionID("user1"); got != b.ID {
		t.Fatalf("active = %q, want %q", got, b.ID)
	}
	if got := sm.backStack["user1"]; len(got) != 1 || got[0] != a.ID {
		t.Fatalf("back stack = %v, want [%q]", got, a.ID)
	}
	if len(sm.sessions) != 2 || len(sm.userSessions["user1"]) != 2 || sm.counter != 2 {
		t.Fatalf("state changed after failed back: sessions=%d userSessions=%d counter=%d", len(sm.sessions), len(sm.userSessions["user1"]), sm.counter)
	}
}

func TestSwitchToAgentSessionWithBackExistingRollsBackOnSaveFailure(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(path)
	userKey := "weixin:user1"
	a := sm.GetOrCreateActive(userKey)
	a.SetAgentInfo("agent-A", "codex", "A")
	b := sm.NewSession(userKey, "B")
	b.SetAgentInfo("agent-B", "codex", "B")
	sm.storePath = t.TempDir()

	got, err := sm.SwitchToAgentSessionWithBack(userKey, "agent-A", "codex", "A")
	if err == nil || got != nil {
		t.Fatalf("SwitchToAgentSessionWithBack = (%v, %v), want (nil, error)", got, err)
	}
	if got := sm.ActiveSessionID(userKey); got != b.ID {
		t.Fatalf("active = %q, want %q", got, b.ID)
	}
	if len(sm.backStack[userKey]) != 0 {
		t.Fatalf("back stack = %v, want empty", sm.backStack[userKey])
	}
	if len(sm.sessions) != 2 || len(sm.userSessions[userKey]) != 2 || sm.counter != 2 {
		t.Fatalf("state changed after failed existing switch: sessions=%d userSessions=%d counter=%d", len(sm.sessions), len(sm.userSessions[userKey]), sm.counter)
	}
}

func TestSwitchToAgentSessionWithBackNewRollsBackOnSaveFailure(t *testing.T) {
	path := filepath.Join(t.TempDir(), "sessions.json")
	sm := NewSessionManager(path)
	userKey := "weixin:user1"
	a := sm.GetOrCreateActive(userKey)
	...
	got, err := sm.SwitchToAgentSessionWithBack(userKey, "agent-B", "codex", "B")
	if err == nil || got != nil {
		t.Fatalf("SwitchToAgentSessionWithBack = (%v, %v), want (nil, error)", got, err)
	}
	if got := sm.ActiveSessionID(userKey); got != a.ID {
		t.Fatalf("active = %q, want %q", got, a.ID)
	}
	if len(sm.backStack[userKey]) != 0 {
		t.Fatalf("back stack = %v, want empty", sm.backStack[userKey])
	}
	if len(sm.sessions) != 1 || len(sm.userSessions[userKey]) != 1 || sm.counter != 1 {
		t.Fatalf("state changed after failed new switch: sessions=%d userSessions=%d counter=%d", len(sm.sessions), len(sm.userSessions[userKey]), sm.counter)
	}
	sm.storePath = ""
	if next := sm.NewSideSession(userKey, "next"); next.ID != "s2" {
		t.Fatalf("next session ID = %q, want s2", next.ID)
	}
}
