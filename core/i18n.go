...
const (
	...
	MsgDoctorCodexCompatibilityName         MsgKey = "doctor_codex_compatibility_name"
	MsgDoctorCodexCompatibilityDisabled     MsgKey = "doctor_codex_compatibility_disabled"
	MsgDoctorCodexCompatibilityIdle         MsgKey = "doctor_codex_compatibility_idle"
	MsgDoctorCodexCompatibilityWaiting      MsgKey = "doctor_codex_compatibility_waiting"
	MsgDoctorCodexCompatibilityHealthy      MsgKey = "doctor_codex_compatibility_healthy"
	MsgDoctorCodexCompatibilityDegraded     MsgKey = "doctor_codex_compatibility_degraded"
	MsgDoctorCodexCompatibilityIncompatible MsgKey = "doctor_codex_compatibility_incompatible"
	...
	MsgNewConversationCreated                    MsgKey = "new_conversation_created"
	MsgNewConversationWelcome                    MsgKey = "new_conversation_welcome"
	MsgNewConversationUnsupported                MsgKey = "new_conversation_unsupported"
	MsgNewConversationSpawnFailed                MsgKey = "new_conversation_spawn_failed"
	MsgNewConversationAgentInitFailed            MsgKey = "new_conversation_agent_init_failed"
	MsgNewConversationAgentInitFailedWelcome     MsgKey = "new_conversation_agent_init_failed_welcome"
	MsgNewConversationAgentInitInProgress        MsgKey = "new_conversation_agent_init_in_progress"
	MsgNewConversationAgentInitInProgressWelcome MsgKey = "new_conversation_agent_init_in_progress_welcome"
	MsgNewConversationNameSyncFailed             MsgKey = "new_conversation_name_sync_failed"
	MsgNewConversationNameSyncFailedWelcome      MsgKey = "new_conversation_name_sync_failed_welcome"
	MsgNewConversationRetryScheduled             MsgKey = "new_conversation_retry_scheduled"
	MsgNewConversationManualAction               MsgKey = "new_conversation_manual_action"
	MsgNewInPlaceCreated                         MsgKey = "new_in_place_created"
	MsgNewInPlaceFailed                          MsgKey = "new_in_place_failed"
	MsgNewInPlaceRetryScheduled                  MsgKey = "new_in_place_retry_scheduled"
	MsgNewInPlaceManualAction                    MsgKey = "new_in_place_manual_action"
	MsgNewInPlaceActiveChanged                   MsgKey = "new_in_place_active_changed"
	...
	MsgSwitchOwnedByOtherConversation MsgKey = "switch_owned_by_other_conversation"
	MsgBackUnsupported                MsgKey = "back_unsupported"
	MsgBackEmpty                      MsgKey = "back_empty"
	MsgBackSuccess                    MsgKey = "back_success"
...
var messages = map[MsgKey]map[Language]string{
	...
	MsgHelp: {
		LangEnglish: "📖 Available Commands\n\n" +
			"/new [name]\n  Feishu: create a new group + Codex session; Weixin: create a Codex session in the same chat\n\n" +
			"/list\n  List agent sessions\n\n" +
			"/search <keyword>\n  Search sessions by name or ID\n\n" +
			"/switch <number>\n  Resume a session by its list number\n\n" +
			"/back\n  Return to the previous logical session (Weixin)\n\n" +
		...
		LangChinese: "📖 可用命令\n\n" +
			"/new [名称]\n  飞书：新建群和 Codex 会话；微信：在原聊天中新建 Codex 会话\n\n" +
			"/list\n  列出 Agent 会话列表\n\n" +
			"/search <关键词>\n  搜索会话名称或 ID\n\n" +
			"/switch <序号>\n  按列表序号切换会话\n\n" +
			"/back\n  返回上一个逻辑会话（微信）\n\n" +
		...
		LangTraditionalChinese: "📖 可用命令\n\n" +
			"/new [名稱]\n  飛書：新建群組和 Codex 會話；微信：在原聊天中新建 Codex 會話\n\n" +
			"/list\n  列出 Agent 會話列表\n\n" +
			"/search <關鍵詞>\n  搜尋會話名稱或 ID\n\n" +
			"/switch <序號>\n  按列表序號切換會話\n\n" +
			"/back\n  返回上一個邏輯會話（微信）\n\n" +
		...
		LangJapanese: "📖 利用可能なコマンド\n\n" +
			"/new [名前]\n  Feishu: 新しいグループと Codex セッションを作成。Weixin: 同じチャット内に Codex セッションを作成\n\n" +
			"/list\n  エージェントセッション一覧\n\n" +
			"/switch <番号>\n  リスト番号でセッションを切り替え\n\n" +
			"/back\n  前の論理セッションに戻る（Weixin）\n\n" +
		...
		LangSpanish: "📖 Comandos disponibles\n\n" +
			"/new [nombre]\n  Feishu: crear un grupo y sesión Codex; Weixin: crear una sesión Codex en el mismo chat\n\n" +
			"/list\n  Listar sesiones del agente\n\n" +
			"/switch <número>\n  Reanudar sesión por su número en la lista\n\n" +
			"/back\n  Volver a la sesión lógica anterior (Weixin)\n\n" +
	...
	MsgDoctorCodexCompatibilityName: {
		LangEnglish:            "Codex App two-way sync compatibility",
		LangChinese:            "Codex App 双向同步兼容性",
		LangTraditionalChinese: "Codex App 雙向同步相容性",
		LangJapanese:           "Codex App 双方向同期の互換性",
		LangSpanish:            "Compatibilidad de sincronización bidireccional de Codex App",
	},
	MsgDoctorCodexCompatibilityDisabled: {
		LangEnglish:            "Disabled (Codex %s).",
		LangChinese:            "未启用（Codex %s）。",
		LangTraditionalChinese: "未啟用（Codex %s）。",
		LangJapanese:           "無効です（Codex %s）。",
		LangSpanish:            "Deshabilitada (Codex %s).",
	},
	MsgDoctorCodexCompatibilityIdle: {
		LangEnglish:            "Idle: no active routes (Codex %s).",
		LangChinese:            "空闲：当前没有活跃路由（Codex %s）。",
		LangTraditionalChinese: "閒置：目前沒有活躍路由（Codex %s）。",
		LangJapanese:           "待機中：アクティブなルートはありません（Codex %s）。",
		LangSpanish:            "Inactiva: no hay rutas activas (Codex %s).",
	},
	MsgDoctorCodexCompatibilityWaiting: {
		LangEnglish:            "Waiting for transcript events: %d active routes (Codex %s).",
		LangChinese:            "正在等待转录事件：%d 条活跃路由（Codex %s）。",
		LangTraditionalChinese: "正在等待轉錄事件：%d 條活躍路由（Codex %s）。",
		LangJapanese:           "トランスクリプトイベントを待機中：アクティブなルート %d 件（Codex %s）。",
		LangSpanish:            "Esperando eventos de transcripción: %d rutas activas (Codex %s).",
	},
	MsgDoctorCodexCompatibilityHealthy: {
		LangEnglish:            "Healthy: supported turn observed at %s (Codex %s).",
		LangChinese:            "正常：已于 %s 识别受支持轮次（Codex %s）。",
		LangTraditionalChinese: "正常：已於 %s 識別受支援輪次（Codex %s）。",
		LangJapanese:           "正常：%s に対応済みターンを確認しました（Codex %s）。",
		LangSpanish:            "Correcta: se observó un turno compatible a las %s (Codex %s).",
	},
	MsgDoctorCodexCompatibilityDegraded: {
		LangEnglish:            "Degraded: %s at %s (Codex %s).",
		LangChinese:            "降级：%s，发生于 %s（Codex %s）。",
		LangTraditionalChinese: "降級：%s，發生於 %s（Codex %s）。",
		LangJapanese:           "機能低下：%s（%s、Codex %s）。",
		LangSpanish:            "Degradada: %s a las %s (Codex %s).",
	},
	MsgDoctorCodexCompatibilityIncompatible: {
		LangEnglish:            "Incompatible: %s at %s; unsafe turns are blocked (Codex %s).",
		LangChinese:            "不兼容：%s，发生于 %s；不安全轮次已拦截（Codex %s）。",
		LangTraditionalChinese: "不相容：%s，發生於 %s；不安全輪次已攔截（Codex %s）。",
		LangJapanese:           "非互換：%s（%s）。安全でないターンは遮断済みです（Codex %s）。",
		LangSpanish:            "Incompatible: %s a las %s; se bloquearon los turnos inseguros (Codex %s).",
	},
	MsgRestarting: {
	...
	MsgNewConversationCreated: {
		LangEnglish:            "✅ Created a new group and Codex session: **%s**. This group and its current session are unchanged.",
		LangChinese:            "✅ 已创建新群和 Codex 会话：**%s**。当前群和当前会话保持不变。",
		LangTraditionalChinese: "✅ 已建立新群組和 Codex 會話：**%s**。目前群組和目前會話保持不變。",
		LangJapanese:           "✅ 新しいグループと Codex セッションを作成しました：**%s**。現在のグループとセッションは変更されません。",
		LangSpanish:            "✅ Se creó un grupo y una sesión de Codex nuevos: **%s**. Este grupo y su sesión actual no cambiaron.",
	},
	MsgNewConversationWelcome: {
		LangEnglish:            "✅ This group is now bound one-to-one to a new Codex session: **%s**.",
		LangChinese:            "✅ 本群已一对一绑定新的 Codex 会话：**%s**。",
		LangTraditionalChinese: "✅ 本群已一對一綁定新的 Codex 會話：**%s**。",
		LangJapanese:           "✅ このグループは新しい Codex セッションに1対1で紐付けられました：**%s**。",
		LangSpanish:            "✅ Este grupo quedó vinculado uno a uno a una nueva sesión de Codex: **%s**.",
	},
	MsgNewConversationRetryScheduled: {
		LangEnglish:            "⏳ Creating **%s** was temporarily interrupted. The system will retry automatically; this group and its current session are unchanged.",
		LangChinese:            "⏳ 新建 **%s** 的流程暂时中断，系统将自动重试；当前群和当前会话保持不变。",
		LangTraditionalChinese: "⏳ 新建 **%s** 的流程暫時中斷，系統將自動重試；目前群組和目前會話保持不變。",
		LangJapanese:           "⏳ **%s** の作成が一時中断されました。システムが自動的に再試行します。現在のグループとセッションは変更されません。",
		LangSpanish:            "⏳ La creación de **%s** se interrumpió temporalmente. El sistema volverá a intentarlo automáticamente; este grupo y su sesión actual no cambiaron.",
	},
	MsgNewConversationManualAction: {
		LangEnglish:            "⚠️ Creating **%s** needs a permission or configuration fix. After fixing it, restart cc-connect to continue; this group and its current session are unchanged.",
		LangChinese:            "⚠️ 新建 **%s** 需要检查权限或配置。修复后重启 cc-connect 即会继续；当前群和当前会话保持不变。",
		LangTraditionalChinese: "⚠️ 新建 **%s** 需要檢查權限或設定。修復後重新啟動 cc-connect 即會繼續；目前群組和目前會話保持不變。",
		LangJapanese:           "⚠️ **%s** の作成には権限または設定の修正が必要です。修正後に cc-connect を再起動すると続行します。現在のグループとセッションは変更されません。",
		LangSpanish:            "⚠️ La creación de **%s** requiere corregir permisos o configuración. Reinicia cc-connect después de corregirlo para continuar; este grupo y su sesión actual no cambiaron.",
	},
	MsgNewInPlaceCreated: {
		LangEnglish:            "✅ Created and switched to a new Codex session: **%s**.",
		LangChinese:            "✅ 已创建并切换到新的 Codex 会话：**%s**。",
		LangTraditionalChinese: "✅ 已建立並切換到新的 Codex 會話：**%s**。",
		LangJapanese:           "✅ 新しい Codex セッションを作成して切り替えました：**%s**。",
		LangSpanish:            "✅ Se creó y activó una nueva sesión de Codex: **%s**.",
	},
	MsgNewInPlaceFailed: {
		LangEnglish:            "❌ Failed to create the new Codex session: %v. The current session was not changed.",
		LangChinese:            "❌ 新 Codex 会话创建失败：%v。当前会话未发生变化。",
		LangTraditionalChinese: "❌ 新 Codex 會話建立失敗：%v。目前會話未發生變化。",
		LangJapanese:           "❌ 新しい Codex セッションを作成できませんでした：%v。現在のセッションは変更されていません。",
		LangSpanish:            "❌ No se pudo crear la nueva sesión de Codex: %v. La sesión actual no cambió.",
	},
	MsgNewInPlaceRetryScheduled: {
		LangEnglish:            "⏳ Creating **%s** was temporarily interrupted. The system will retry automatically; the current session is unchanged.",
		LangChinese:            "⏳ 新建 **%s** 暂时中断，当前会话保持不变；系统将自动重试。",
		LangTraditionalChinese: "⏳ 新建 **%s** 暫時中斷，目前會話保持不變；系統將自動重試。",
		LangJapanese:           "⏳ **%s** の作成が一時中断されました。現在のセッションは変更せず、システムが自動的に再試行します。",
		LangSpanish:            "⏳ La creación de **%s** se interrumpió temporalmente. La sesión actual no cambió; el sistema volverá a intentarlo automáticamente.",
	},
	MsgNewInPlaceManualAction: {
		LangEnglish:            "⚠️ Creating **%s** needs a permission or configuration fix. The current session is unchanged.",
		LangChinese:            "⚠️ 新建 **%s** 需要检查权限或配置，当前会话保持不变。",
		LangTraditionalChinese: "⚠️ 新建 **%s** 需要檢查權限或設定，目前會話保持不變。",
		LangJapanese:           "⚠️ **%s** の作成には権限または設定の確認が必要です。現在のセッションは変更されていません。",
		LangSpanish:            "⚠️ La creación de **%s** requiere revisar permisos o configuración. La sesión actual no cambió.",
	},
	MsgNewInPlaceActiveChanged: {
		LangEnglish:            "⚠️ **%s** is ready, but you switched sessions while it was being restored. Your current session was kept. Use /list and then /switch <number> to activate it explicitly.",
		LangChinese:            "⚠️ **%s** 已准备好，但恢复期间你已切换到其他会话；系统保留了当前选择。请先用 /list 查看，再用 /switch <序号> 显式切换。",
		LangTraditionalChinese: "⚠️ **%s** 已準備好，但恢復期間你已切換到其他會話；系統保留了目前選擇。請先用 /list 查看，再用 /switch <序號> 明確切換。",
		LangJapanese:           "⚠️ **%s** の準備は完了しましたが、復元中に別のセッションへ切り替えられました。現在の選択を維持しています。/list で確認し、/switch <番号> で明示的に切り替えてください。",
		LangSpanish:            "⚠️ **%s** está lista, pero cambiaste de sesión durante la recuperación. Se conservó la sesión actual. Usa /list y después /switch <número> para activarla explícitamente.",
	},
	MsgNewConversationAgentInitInProgress: {
		LangEnglish:            "⏳ The new group **%s** was created. Its first message is initializing the Codex session; it will appear in Codex after that turn finishes. This group is unchanged.",
		LangChinese:            "⏳ 新群 **%s** 已创建；检测到首条消息正在初始化 Codex 会话，处理完成后会自动显示在 Codex。当前群保持不变。",
		LangTraditionalChinese: "⏳ 新群組 **%s** 已建立；偵測到第一則訊息正在初始化 Codex 會話，處理完成後會自動顯示在 Codex。目前群組保持不變。",
		LangJapanese:           "⏳ 新しいグループ **%s** を作成しました。最初のメッセージで Codex セッションを初期化中で、処理完了後に Codex に表示されます。現在のグループは変更されません。",
		LangSpanish:            "⏳ Se creó el grupo nuevo **%s**. Su primer mensaje está inicializando la sesión de Codex; aparecerá en Codex cuando termine. Este grupo no cambió.",
	},
	MsgNewConversationAgentInitInProgressWelcome: {
		LangEnglish:            "⏳ This group was created for **%s**. Its first message is initializing the Codex session; it will appear in Codex after that turn finishes.",
		LangChinese:            "⏳ 本群已为 **%s** 创建；首条消息正在初始化 Codex 会话，处理完成后会自动显示在 Codex。",
		LangTraditionalChinese: "⏳ 本群已為 **%s** 建立；第一則訊息正在初始化 Codex 會話，處理完成後會自動顯示在 Codex。",
		LangJapanese:           "⏳ このグループは **%s** 用に作成されました。最初のメッセージで Codex セッションを初期化中で、処理完了後に Codex に表示されます。",
		LangSpanish:            "⏳ Este grupo se creó para **%s**. Su primer mensaje está inicializando la sesión de Codex; aparecerá en Codex cuando termine.",
	},
	MsgNewConversationUnsupported: {
		LangEnglish:            "❌ This platform does not support creating a new group with `/new`. This group and its current session were not changed.",
		LangChinese:            "❌ 当前平台不支持使用 `/new` 创建新群。当前群和当前会话未发生变化。",
		LangTraditionalChinese: "❌ 目前平台不支援使用 `/new` 建立新群組。目前群組和目前會話未發生變化。",
		LangJapanese:           "❌ このプラットフォームでは `/new` による新規グループ作成をサポートしていません。現在のグループとセッションは変更されていません。",
		LangSpanish:            "❌ Esta plataforma no admite crear un grupo nuevo con `/new`. Este grupo y su sesión actual no cambiaron.",
	},
	MsgNewConversationSpawnFailed: {
		LangEnglish:            "❌ Failed to create the new group: %v. This group and its current session were not changed.",
		LangChinese:            "❌ 新群创建失败：%v。当前群和当前会话未发生变化。",
		LangTraditionalChinese: "❌ 新群組建立失敗：%v。目前群組和目前會話未發生變化。",
		LangJapanese:           "❌ 新しいグループを作成できませんでした：%v。現在のグループとセッションは変更されていません。",
		LangSpanish:            "❌ No se pudo crear el grupo nuevo: %v. Este grupo y su sesión actual no cambiaron.",
	},
	MsgNewConversationAgentInitFailed: {
		LangEnglish:            "⚠️ The new group **%s** was created, but its Codex session could not be initialized. Send a message in the new group to retry. This group is unchanged.",
		LangChinese:            "⚠️ 新群 **%s** 已创建，但 Codex 会话初始化失败。请在新群发送消息重试；当前群保持不变。",
		LangTraditionalChinese: "⚠️ 新群組 **%s** 已建立，但 Codex 會話初始化失敗。請在新群組傳送訊息重試；目前群組保持不變。",
		LangJapanese:           "⚠️ 新しいグループ **%s** は作成されましたが、Codex セッションを初期化できませんでした。新しいグループでメッセージを送信して再試行してください。現在のグループは変更されていません。",
		LangSpanish:            "⚠️ Se creó el grupo nuevo **%s**, pero no se pudo inicializar su sesión de Codex. Envía un mensaje en el grupo nuevo para reintentar. Este grupo no cambió.",
	},
	MsgNewConversationAgentInitFailedWelcome: {
		LangEnglish:            "⚠️ This group was created for **%s**, but its Codex session could not be initialized. Send a message here to retry.",
		LangChinese:            "⚠️ 本群已为 **%s** 创建，但 Codex 会话初始化失败。请在本群发送消息重试。",
		LangTraditionalChinese: "⚠️ 本群組已為 **%s** 建立，但 Codex 會話初始化失敗。請在本群組傳送訊息重試。",
		LangJapanese:           "⚠️ このグループは **%s** 用に作成されましたが、Codex セッションを初期化できませんでした。ここでメッセージを送信して再試行してください。",
		LangSpanish:            "⚠️ Este grupo se creó para **%s**, pero no se pudo inicializar su sesión de Codex. Envía un mensaje aquí para reintentar.",
	},
	MsgNewConversationNameSyncFailed: {
		LangEnglish:            "⚠️ The new group and Codex session **%s** were created, but the Codex task name could not be synchronized. Send a message in the new group to retry. This group is unchanged.",
		LangChinese:            "⚠️ 新群和 Codex 会话 **%s** 已创建，但 Codex 任务名称同步失败。请在新群发送消息重试；当前群保持不变。",
		LangTraditionalChinese: "⚠️ 新群組和 Codex 會話 **%s** 已建立，但 Codex 任務名稱同步失敗。請在新群組傳送訊息重試；目前群組保持不變。",
		LangJapanese:           "⚠️ 新しいグループと Codex セッション **%s** は作成されましたが、Codex タスク名を同期できませんでした。新しいグループでメッセージを送信して再試行してください。現在のグループは変更されていません。",
		LangSpanish:            "⚠️ Se crearon el grupo y la sesión de Codex **%s**, pero no se pudo sincronizar el nombre de la tarea. Envía un mensaje en el grupo nuevo para reintentar. Este grupo no cambió.",
	},
	MsgNewConversationNameSyncFailedWelcome: {
		LangEnglish:            "⚠️ This group is bound to a new Codex session, but the task name **%s** could not be synchronized. Send a message here to retry.",
		LangChinese:            "⚠️ 本群已绑定新的 Codex 会话，但任务名称 **%s** 同步失败。请在本群发送消息重试。",
		LangTraditionalChinese: "⚠️ 本群組已綁定新的 Codex 會話，但任務名稱 **%s** 同步失敗。請在本群組傳送訊息重試。",
		LangJapanese:           "⚠️ このグループは新しい Codex セッションに紐付けられましたが、タスク名 **%s** を同期できませんでした。ここでメッセージを送信して再試行してください。",
		LangSpanish:            "⚠️ Este grupo está vinculado a una sesión de Codex nueva, pero no se pudo sincronizar el nombre **%s**. Envía un mensaje aquí para reintentar.",
	},
	...
	MsgSwitchOwnedByOtherConversation: {
		LangEnglish:            "❌ This Codex session is already bound to another conversation and cannot be switched here.",
		LangChinese:            "❌ 该 Codex 会话已绑定其他群，不能切换到当前群。",
		LangTraditionalChinese: "❌ 該 Codex 會話已綁定其他群組，不能切換到目前群組。",
		LangJapanese:           "❌ この Codex セッションは別のグループに紐付け済みのため、ここへ切り替えられません。",
		LangSpanish:            "❌ Esta sesión de Codex ya está vinculada a otro grupo y no puede cambiarse aquí.",
	},
	MsgBackUnsupported: {
		LangEnglish:            "❌ This platform does not support returning to the previous logical session.",
		LangChinese:            "❌ 当前平台不支持返回上一个逻辑会话。",
		LangTraditionalChinese: "❌ 目前平台不支援返回上一個邏輯會話。",
		LangJapanese:           "❌ このプラットフォームでは前の論理セッションに戻れません。",
		LangSpanish:            "❌ Esta plataforma no permite volver a la sesión lógica anterior.",
	},
	MsgBackEmpty: {
		LangEnglish:            "ℹ️ There is no previous session to return to.",
		LangChinese:            "ℹ️ 没有可返回的上一个会话。",
		LangTraditionalChinese: "ℹ️ 沒有可返回的上一個會話。",
		LangJapanese:           "ℹ️ 戻ることのできる前のセッションはありません。",
		LangSpanish:            "ℹ️ No hay una sesión anterior a la que volver.",
	},
	MsgBackSuccess: {
		LangEnglish:            "✅ Returned to: %s",
		LangChinese:            "✅ 已返回：%s",
		LangTraditionalChinese: "✅ 已返回：%s",
		LangJapanese:           "✅ 戻りました：%s",
		LangSpanish:            "✅ Se volvió a: %s",
	},
	MsgCommandTimeout: {
	...
	MsgBuiltinCmdNew: {
		LangEnglish:            "Create a new group + Codex session on Feishu, or a new Codex session in the same Weixin chat; arg: [name]",
		LangChinese:            "飞书中新建群和 Codex 会话；微信原聊天中新建 Codex 会话，参数: [名称]",
		LangTraditionalChinese: "飛書中新建群組和 Codex 會話；微信原聊天中新建 Codex 會話，參數: [名稱]",
		LangJapanese:           "Feishu では新しいグループと Codex セッションを、Weixin では同じチャット内に新しい Codex セッションを作成。引数: [名前]",
		LangSpanish:            "Crea un grupo y sesión Codex en Feishu, o una sesión Codex en el mismo chat de Weixin; arg: [nombre]",
	},
	MsgBuiltinCmdList: {
	...
	MsgBuiltinCmdBack: {
		LangEnglish:            "Return to the previous logical session (Weixin)",
		LangChinese:            "返回上一个逻辑会话（微信）",
		LangTraditionalChinese: "返回上一個邏輯會話（微信）",
		LangJapanese:           "前の論理セッションに戻る（Weixin）",
		LangSpanish:            "Volver a la sesión lógica anterior (Weixin)",
	},
	MsgBuiltinCmdDelete: {
