...
// ConversationSpawnRequest describes a request to create a new platform
// conversation that will be bound to its own agent session.
type ConversationSpawnRequest struct {
	Name      string
	UserID    string
	MessageID string
}

// SpawnedConversation identifies the new platform target created by a
// ConversationSpawner.
type SpawnedConversation struct {
	SessionKey string
	ReplyCtx   any
}

// ConversationSpawner is an optional platform capability used by /new to
// create a separate chat without changing the conversation that invoked it.
type ConversationSpawner interface {
...
type InPlaceConversationForker interface {
	SupportsInPlaceConversationFork() bool
}

// ConversationNameFormatter is an optional agent capability used by /new to
// apply an agent-specific display name before the platform conversation and
// agent session are created.
type ConversationNameFormatter interface {
	FormatConversationName(name string) string
}

// ConversationSourceNameFormatter is an optional agent capability used by
// /new when display names vary by the originating platform. The legacy
// ConversationNameFormatter remains the fallback for agents without it.
type ConversationSourceNameFormatter interface {
	FormatConversationNameForPlatform(platformName, name string) string
}
...
// ErrSessionContextPreservationRequired indicates that a saved agent session
// still exists, but could not be resumed safely. Callers must not replace it
// with a fresh session because that would discard the saved conversation.
var ErrSessionContextPreservationRequired = errors.New("session context must be preserved")
...
// SessionReplyPrefixer is an optional platform capability for labeling text
// replies with the active logical session name.
type SessionReplyPrefixer interface {
	ReplySessionKey(replyCtx any) string
	FormatSessionReply(sessionName, content string) string
}
...
For images: when you generate a local image that should be delivered, use:

  cc-connect send --image /absolute/path/to/image.png

For any regular file, whether it appears in a Codex App Files block or you generated it, use --file only after the user explicitly asks to send or deliver that file to the current chat, for example "send this file to the current chat":

  cc-connect send --file /absolute/path/to/report.pdf
  cc-connect send --file /absolute/path/to/report.pdf --image /absolute/path/to/chart.png

You may repeat --image / --file multiple times. Never decide on your own that a regular file should be sent.
...
// RecoverableAgentSessionStarter is an optional agent capability for creating
// a session with a stable key that can be rediscovered after a process restart.
type RecoverableAgentSessionStarter interface {
...
// that can tail messages created by their desktop application.
type ExternalConversationPoller interface {
	PollExternalConversation(ctx context.Context, sessionID string) ([]ExternalConversationEvent, error)
}

// ExternalConversationRouteTracker lets a poller discard cursors for routes
// that are no longer active, so messages written while inactive are not replayed
// if the route is activated again later.
type ExternalConversationRouteTracker interface {
	SetExternalConversationRoutes(sessionIDs []string)
}

// ExternalConversationRelayTarget marks a platform as an allowed destination
// for agent-owned conversation events. Keeping this as an explicit capability
// prevents core from hardcoding platform names or relaying desktop events to
// every platform that happens to support proactive replies.
type ExternalConversationRelayTarget interface {
	ExternalConversationRelayEnabled() bool
}
...
// AgentSessionNamer is an optional capability for agent backends that can
// persist a human-readable name for a conversation.
type AgentSessionNamer interface {
...
type AgentSessionPrimer interface {
	PrimeSession(name string) error
}

// AgentSessionArchiver is an optional capability for agent backends that can
// archive an older conversation after a replacement session is established.
type AgentSessionArchiver interface {
