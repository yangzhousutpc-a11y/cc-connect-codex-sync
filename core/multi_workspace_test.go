...
type inPlaceWorkspacePlatform struct {
	*mockChannelResolver
}

func (p *inPlaceWorkspacePlatform) SupportsInPlaceConversationFork() bool { return true }
...
func TestWeixinNavigationRestoresOriginalWorkspace(t *testing.T) {
	...
	bindingPath := filepath.Join(stateDir, "bindings.json")
	const agentName = "weixin-navigation-workspace-agent"
	RegisterAgent(agentName, func(map[string]any) (Agent, error) {
		return &namedTestAgent{name: agentName}, nil
	})
	p := &inPlaceWorkspacePlatform{mockChannelResolver: &mockChannelResolver{name: "weixin", names: map[string]string{}}}
	const channelID = "dm-peer"
	const sessionKey = "weixin:dm-peer:peer"
	msg := &Message{Platform: "weixin", ChannelKey: channelID, SessionKey: sessionKey, UserID: "peer", ReplyCtx: "ctx"}

	e := NewEngine("test", &namedTestAgent{name: agentName}, []Platform{p}, sessionPath, LangChinese)
	e.SetMultiWorkspace(baseDir, bindingPath)
	e.SetWorkspaceIdleTimeout(0)
	e.workspaceBindings.Bind("project:test", workspaceChannelKey("weixin", channelID), "workspace-1", workspaceDir)
	_, sessions, _, gotWorkspace, err := e.commandContextWithWorkspace(p, msg)
	if err != nil {
		t.Fatal(err)
	}
	if gotWorkspace != normalizeWorkspacePath(workspaceDir) {
		t.Fatalf("workspace = %q, want %q", gotWorkspace, normalizeWorkspacePath(workspaceDir))
	}
	a := sessions.NewSession(sessionKey, "[Codex] A")
	a.SetAgentSessionID("thread-a", agentName)
	b := sessions.NewSideSession(sessionKey, "[Codex] B")
	b.SetAgentSessionID("thread-b", agentName)
	if err := sessions.ActivatePreparedSession(sessionKey, b.ID, a.ID); err != nil {
		t.Fatal(err)
	}
	...
	restarted := NewEngine("test", &namedTestAgent{name: agentName}, []Platform{p}, sessionPath, LangChinese)
	restarted.SetMultiWorkspace(baseDir, bindingPath)
	restarted.SetWorkspaceIdleTimeout(0)
	t.Cleanup(func() { _ = restarted.Stop() })
	msg.MessageID = "m-back"
	msg.Content = "/back"
	restarted.ReceiveMessage(p, msg)

	_, restoredSessions, _, restoredWorkspace, err := restarted.commandContextWithWorkspace(p, msg)
	if err != nil {
		t.Fatal(err)
	}
	if got := restoredSessions.GetOrCreateActive(sessionKey).GetAgentSessionID(); got != "thread-a" {
		t.Fatalf("active thread after restart/back = %q, want thread-a", got)
	}
	if restoredWorkspace != normalizeWorkspacePath(workspaceDir) {
		t.Fatalf("restored workspace = %q, want %q", restoredWorkspace, normalizeWorkspacePath(workspaceDir))
	}
}
