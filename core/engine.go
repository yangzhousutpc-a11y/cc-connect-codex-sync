...
func (e *Engine) dispatchRestartNotify(req *RestartRequest) error {
	...
	for attempt, wait := range backoffs {
		...
		content := e.decorateSessionReply(p, rctx, text)
		if err := p.Send(e.ctx, rctx, content); err != nil {
			lastErr = err
...
type Engine struct {
	...
	newOperations          *NewOperationStore
	newOperationMu         sync.Mutex
	newOperationRun        map[string]bool
	newOperationTimer      map[string]*time.Timer
	newOperationRetryDelay time.Duration
	conversationNow        func() time.Time
	inPlaceTransitionMu    sync.Mutex
	inPlaceTransitions     map[string]*inPlaceTransition
	...
	// agentStartLocks keep mutable per-start configuration paired with the
	// StartSession call that consumes it, without serializing distinct agents.
	agentStartLocksMu  sync.Mutex
...
type queuedMessage struct {
	...
	inPlaceCompletion *inPlaceMessageCompletion
}
...
type interactiveState struct {
	...
	startupErr               error
	starting                 bool // true while the route's initial agent session is being created
...
func NewEngine(name string, ag Agent, platforms []Platform, sessionStorePath string, lang Language) *Engine {
	...
	e := &Engine{
		...
		platformOperationCtx:    make(map[Platform]context.Context),
		platformOperationCancel: make(map[Platform]context.CancelFunc),
		newOperationRun:         make(map[string]bool),
		newOperationTimer:       make(map[string]*time.Timer),
		newOperationRetryDelay:  newOperationRetryDelay,
		conversationNow:         time.Now,
		inPlaceTransitions:      make(map[string]*inPlaceTransition),
		startedAt:               time.Now(),
...
func (e *Engine) Stop() error {
	...
	for id, timer := range e.newOperationTimer {
		timer.Stop()
...
func (e *Engine) onPlatformReady(p Platform) {
	...
	go e.recoverNewOperationsForPlatform(p)
}
...
func (e *Engine) markPlatformReady(p Platform) bool {
	...
	if cancel := e.platformOperationCancel[p]; cancel != nil {
		cancel()
	}
	readyCtx, cancel := context.WithCancel(e.ctx)
	e.platformOperationCtx[p] = readyCtx
	e.platformOperationCancel[p] = cancel
	return true
}
...
func (e *Engine) markPlatformUnavailable(p Platform) bool {
	...
	delete(e.platformOperationCtx, p)
	delete(e.platformOperationCancel, p)
	return true
}
...
func (e *Engine) handleMessage(p Platform, msg *Message) {
	e.handleMessageWithInPlaceTransition(p, msg, nil)
}

func (e *Engine) handleMessageWithInPlaceTransition(p Platform, msg *Message, bypass *inPlaceTransition) {
	completionHandedOff := false
	defer func() {
		if !completionHandedOff {
			msg.inPlaceCompletion.complete()
		}
	}()
	defer e.finishInPlaceControl(msg)
	if msg.Recalled {
		e.handleMessageRecall(p, msg)
		return
	}
	if e.newOperations != nil && !e.newOperations.WaitUntilMessageRecoverySafe(e.ctx, p.Name(), msg.SessionKey) {
		return
	}
	ownedTransition, queued := e.prepareInPlaceDispatch(p, msg, bypass)
	if msg.inPlaceControlTransition == nil {
		msg.acknowledgeDispatchAdmission()
	}
	if queued {
		return
	}
	if ownedTransition != nil {
		defer func() {
			e.finishInPlaceTransition(msg.SessionKey, ownedTransition)
			if msg.inPlaceTransition == ownedTransition {
				msg.inPlaceTransition = nil
			}
		}()
	}
	...
	permissionHandled := e.handlePendingPermission(p, msg, content, interactiveKey)
	e.finishInPlaceControl(msg)
	...
	if !session.TryLock() {
		...
		if handled, completionDeferred := e.queueMessageForBusySessionInternal(p, msg, interactiveKey); handled {
			completionHandedOff = completionDeferred
...
sessionLocked:
	...
	if msg.inPlaceCompletion != nil {
		completionHandedOff = true
		completion := msg.inPlaceCompletion
		go func() {
...
func (e *Engine) queueMessageForBusySession(p Platform, msg *Message, interactiveKey string) bool {
	handled, _ := e.queueMessageForBusySessionInternal(p, msg, interactiveKey)
	return handled
}

func (e *Engine) queueMessageForBusySessionInternal(p Platform, msg *Message, interactiveKey string) (bool, bool) {
	e.interactiveMu.Lock()
	state, hasState := e.interactiveStates[interactiveKey]
	...
	state.mu.Lock()
	e.interactiveMu.Unlock()
	// Reply decoration may resolve a multi-workspace session through
	// interactiveMu. All user replies below must happen after state.mu unlocks.
	...
	state.pendingMessages = append(state.pendingMessages, queuedMessage{
		...
		inPlaceCompletion: msg.inPlaceCompletion,
	})
	...
	return true, msg.inPlaceCompletion != nil
}
...
func (e *Engine) processInteractiveMessageWith(p Platform, msg *Message, session *Session, agent Agent, sessions *SessionManager, interactiveKey string, workspaceDir string, ccSessionKey string) {
	...
	if !e.newOperationManagesTargetName(msg.SessionKey) {
		if err := syncAgentSessionName(state.agentSession, agentSessionName); err != nil {
			slog.Warn("failed to sync agent session name", "name", agentSessionName, "error", err)
		}
	}
	...
	archived, err := archivePendingAgentSession(as, session)
	if err != nil {
		slog.Warn("failed to archive replaced agent session", "session_id", session.GetPendingArchiveAgentSessionID(), "error", err)
	} else if archived {
		sessions.Save()
	}
	if !e.newOperationManagesTargetName(msg.SessionKey) {
		if err := syncAgentSessionName(as, agentSessionName); err != nil {
			slog.Warn("failed to refresh agent session name after turn", "name", agentSessionName, "error", err)
		}
	}
...
func syncAgentSessionName(session AgentSession, name string) error {
	name = strings.TrimSpace(name)
	if name == "" {
		return nil
	}
	if namer, ok := session.(AgentSessionNamer); ok {
		return namer.SetSessionName(name)
	}
	return nil
}
...
func archivePendingAgentSession(agentSession AgentSession, session *Session) (bool, error) {
	sessionID := session.GetPendingArchiveAgentSessionID()
	if sessionID == "" {
		return false, nil
	}
	archiver, ok := agentSession.(AgentSessionArchiver)
	if !ok {
		session.ClearPendingArchiveAgentSessionID(sessionID)
		return true, nil
	}
	if err := archiver.ArchiveSession(sessionID); err != nil {
		return false, err
	}
	session.ClearPendingArchiveAgentSessionID(sessionID)
	return true, nil
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	targetSessionKey := ccSessionKey
	if targetSessionKey == "" {
		targetSessionKey = sessionKey
	}
	recoveryKey := ""
	if e.newOperations != nil {
		recoveryKey = e.newOperations.RecoveryKeyForTarget(targetSessionKey)
	}
	return e.getOrCreateInteractiveStateWithRecoveryContext(e.ctx, sessionKey, p, replyCtx, session, sessions, agentOverride, ccSessionKey, recoveryKey, true)
}

// mutableAgentStartLock returns a per-agent lock only when this startup mutates
// agent-level configuration. Agents with no mutable startup configuration keep
// their existing ability to start independent routes concurrently.
func (e *Engine) mutableAgentStartLock(agent Agent, session *Session) *sync.Mutex {
	_, injectsEnv := agent.(SessionEnvInjector)
	_, injectsPrompt := agent.(PlatformPromptInjector)
	_, switchesProvider := agent.(ProviderSwitcher)
	if !injectsEnv && !injectsPrompt && (!switchesProvider || session == nil || session.GetActiveProvider() == "") {
		return nil
	}
	if agent == nil || !reflect.TypeOf(agent).Comparable() {
		return &e.agentStartFallback
	}
	e.agentStartLocksMu.Lock()
	defer e.agentStartLocksMu.Unlock()
	if e.agentStartLocks == nil {
		e.agentStartLocks = make(map[Agent]*sync.Mutex)
	}
	if lock := e.agentStartLocks[agent]; lock != nil {
		return lock
	}
	lock := &sync.Mutex{}
	e.agentStartLocks[agent] = lock
	return lock
}

func (e *Engine) getOrCreateInteractiveStateWithRecoveryContext(requestCtx context.Context, sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey, recoveryKey string, allowFreshFallback bool) *interactiveState {
	...
	// Phase one: install one stable placeholder per route. Starting an agent can
	// make app-server RPCs, disk writes, or process shutdowns block for minutes,
	// so none of those operations may run while interactiveMu is held.
	var state *interactiveState
	for {
		e.interactiveMu.Lock()
		existing := e.interactiveStates[sessionKey]
		if existing == nil {
			state = &interactiveState{
				starting:         true,
		...
		existing.mu.Lock()
		existingAgent := existing.agentSession
		starting := existing.starting
		startDone := existing.startDone
		stopped := existing.stopped
		startupErr := existing.startupErr
		existing.mu.Unlock()
		e.interactiveMu.Unlock()
		...
		if existingAgent == nil {
			// A queueing placeholder, or a failed earlier start, is replaced by
			// this route's one real starter. Keep its queued messages below.
			e.interactiveMu.Lock()
			if e.interactiveStates[sessionKey] != existing {
				e.interactiveMu.Unlock()
				continue
			}
			state = &interactiveState{
			...
			adoptPendingFromPlaceholder(existing, state)
			e.interactiveStates[sessionKey] = state
			e.interactiveMu.Unlock()
			break
		}
		...
		// After a same-key session replacement or /switch the active session changes,
		// recycle the old process before starting the replacement.
		wantID := session.GetAgentSessionID()
		currentID := existingAgent.CurrentSessionID()
		needRecycle := currentID != "" && (wantID == "" || wantID != currentID)
		if !needRecycle && existingAgent.Alive() {
			return existing
		}
		if needRecycle {
			...
			slog.Debug("retrying failed interactive session start", "session_key", sessionKey, "error", startupErr)
		}
		// cleanupInteractiveState deliberately closes outside interactiveMu. It
		// also retains the map entry until close finishes, so a concurrent route
		// cannot start a second session while this one is shutting down.
		e.cleanupInteractiveState(sessionKey, existing)
	}
	...
	// Agent startup configuration is mutable on several agent implementations.
	// Keep configuration and the StartSession call that consumes it atomic so
	// concurrent first messages in two groups cannot exchange env/provider data.
	startupLock := e.mutableAgentStartLock(agent, session)
	if startupLock != nil {
		startupLock.Lock()
	}
	...
	if requestCtx.Err() != nil {
		...
		e.finishInteractiveStart(sessionKey, state, nil, requestCtx.Err())
		return state
	}
	...
	if startSessionID != "" {
		if validator, ok := agent.(SessionIDValidator); ok && !validator.ValidateSessionID(requestCtx, startSessionID) {
	...
	startAt := time.Now()
	startAgentSession := func(sessionID string) (AgentSession, error) {
		if recoveryKey != "" {
			if starter, ok := agent.(RecoverableAgentSessionStarter); ok {
				return starter.StartRecoverableSession(requestCtx, sessionID, recoveryKey)
			}
		}
		return agent.StartSession(requestCtx, sessionID)
	}
	agentSession, err := startAgentSession(startSessionID)
	startElapsed := time.Since(startAt)
	if err != nil {
		// If resume/continue failed, try a fresh session as fallback.
		if startSessionID != "" && allowFreshFallback && !errors.Is(err, ErrSessionContextPreservationRequired) {
	...
	if err != nil {
		...
		e.finishInteractiveStart(sessionKey, state, nil, err)
		return state
	}
	...
	if !e.finishInteractiveStart(sessionKey, state, agentSession, nil) {
		// The route was reset while StartSession was running. Do not leave an
		// orphan subprocess behind, and do not resurrect the removed state.
...
// finishInteractiveStart publishes a completed starter only when its
// placeholder is still the current route entry. It never calls agent RPCs or
// persistence while holding the interactive-state locks.
func (e *Engine) finishInteractiveStart(sessionKey string, state *interactiveState, agentSession AgentSession, startupErr error) bool {
	e.interactiveMu.Lock()
...
// a stale goroutine from accidentally destroying the replacement state.
//
...
func (e *Engine) drainPendingMessages(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string) bool {
	for {
		...
		droppedStale := 0
		for len(state.pendingMessages) > 0 && e.isQueuedUserMessageStaleForDrainLocked(state, state.pendingMessages[0].userMessageTimeMs) {
			state.pendingMessages[0].inPlaceCompletion.complete()
			state.pendingMessages = state.pendingMessages[1:]
			droppedStale++
		}
		if droppedStale > 0 {
		...
		if as == nil || !as.Alive() {
			...
			queued.inPlaceCompletion.complete()
			e.notifyDroppedQueuedMessages(state, fmt.Errorf("agent session ended"))
			return false
		}
...
func (e *Engine) cmdNew(p Platform, msg *Message, args []string) {
	...
	if !ok {
		forker, supportsInPlace := p.(InPlaceConversationForker)
		if !supportsInPlace || !forker.SupportsInPlaceConversationFork() {
			e.reply(p, msg.ReplyCtx, e.i18n.T(MsgNewConversationUnsupported))
			return
		}
		if msg.inPlaceTransition == nil {
			transition, owner := e.beginOrQueueInPlaceTransition(p, msg)
			if !owner {
				return
			}
			defer e.finishInPlaceTransition(msg.SessionKey, transition)
		}
		name := e.newConversationName(p.Name(), agent, msg, args, true)
		e.startInPlaceNewOperation(p, msg, agent, sessions, workspaceDir, name)
		return
	}

	name := e.newConversationName(p.Name(), agent, msg, args, false)
	if e.newOperations != nil {
		if strings.TrimSpace(msg.MessageID) == "" {
			err := fmt.Errorf("source message ID is required for recoverable /new")
			e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationSpawnFailed, err))
			return
		}
		opID := NewOperationID(e.name, msg.SessionKey, msg.MessageID)
		op, _, createErr := e.newOperations.CreateOrGet(NewOperation{
			ID:               opID,
			Project:          e.name,
			Platform:         p.Name(),
		...
		if createErr != nil {
			e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationSpawnFailed, createErr))
			return
		}
		if op.Status == NewOperationManualAction {
			op, createErr = e.newOperations.Update(op.ID, func(next *NewOperation) error {
		...
		e.advanceNewOperation(p, op.ID, msg.ReplyCtx, agent, sessions)
		return
	}

	spawned, err := spawner.SpawnConversation(e.ctx, ConversationSpawnRequest{
		Name:      name,
		UserID:    msg.UserID,
		MessageID: msg.MessageID,
	})
	...
	if strings.TrimSpace(spawned.SessionKey) == "" || spawned.ReplyCtx == nil || spawned.SessionKey == msg.SessionKey {
		err := fmt.Errorf("platform returned an invalid conversation target")
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationSpawnFailed, err))
		return
	}
	if e.multiWorkspace && workspaceDir != "" && e.workspaceBindings != nil {
		channelKey := extractWorkspaceChannelKey(spawned.SessionKey)
		if channelKey != "" {
			e.workspaceBindings.Bind("project:"+e.name, channelKey, name, normalizeWorkspacePath(workspaceDir))
		}
	}

	// GetOrCreateActive makes repeated delivery of the same /new idempotent when
	// the platform returns the same group for the same message ID.
	newSession := sessions.GetOrCreateActive(spawned.SessionKey)
	newSession.SetName(name)
	sessions.Save()
	if !newSession.TryLock() {
		// A message in the newly created group won the race and already owns the
		// session. Do not start a second turn or consume its event stream; that
		// real user turn will make the Codex task visible.
		e.send(p, spawned.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitInProgressWelcome, name))
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitInProgress, name))
		return
	}

	interactiveKey := spawned.SessionKey
	if workspaceDir != "" {
		interactiveKey = workspaceDir + ":" + spawned.SessionKey
	}
	...
	defer e.releaseSpawnedSessionAfterInitialization(state, newSession, sessions, interactiveKey, agent, workspaceDir)

	if state.agentSession == nil || newSession.GetAgentSessionID() == "" {
		e.send(p, spawned.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitFailedWelcome, name))
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitFailed, name))
		return
	}
	if err := primeAgentSessionVisibility(state.agentSession, name); err != nil {
		slog.Warn("failed to initialize spawned agent session visibility", "name", name, "error", err)
		e.send(p, spawned.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitFailedWelcome, name))
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationAgentInitFailed, name))
		return
	}
	if err := syncAgentSessionName(state.agentSession, name); err != nil {
		slog.Warn("failed to sync spawned agent session name", "name", name, "error", err)
		e.send(p, spawned.ReplyCtx, e.i18n.Tf(MsgNewConversationNameSyncFailedWelcome, name))
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationNameSyncFailed, name))
		return
	}

	e.send(p, spawned.ReplyCtx, e.i18n.Tf(MsgNewConversationWelcome, name))
	e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgNewConversationCreated, name))
}

func (e *Engine) newConversationName(platformName string, agent Agent, msg *Message, args []string, inPlace bool) string {
	name := strings.TrimSpace(strings.Join(args, " "))
	if name == "" {
		if inPlace {
			now := time.Now
			if e.conversationNow != nil {
				now = e.conversationNow
			}
			beijing := time.FixedZone("CST", 8*60*60)
			name = "微信新会话-" + now().In(beijing).Format("0102-1504")
		} else if sourceName := strings.TrimSpace(msg.ChatName); sourceName != "" {
			name = sourceName + "-新会话"
		} else {
			name = "新会话"
		}
	...
	if formatter, ok := agent.(ConversationSourceNameFormatter); ok {
		if formatted := strings.TrimSpace(formatter.FormatConversationNameForPlatform(platformName, name)); formatted != "" {
			return formatted
		}
	}
	if formatter, ok := agent.(ConversationNameFormatter); ok {
		if formatted := strings.TrimSpace(formatter.FormatConversationName(name)); formatted != "" {
			name = formatted
		}
	}
	return name
...
// releaseSpawnedSessionAfterInitialization atomically opens the new session
// after thread creation and name sync. Messages that arrived while /new was
// initializing are handed to the normal queued-message drain.
func (e *Engine) releaseSpawnedSessionAfterInitialization(state *interactiveState, session *Session, sessions *SessionManager, interactiveKey string, agent Agent, workspaceDir string) {
	state.mu.Lock()
	if len(state.pendingMessages) == 0 {
		session.Unlock()
		state.mu.Unlock()
	...
	state.mu.Unlock()
	go e.drainOrphanedQueue(session, sessions, interactiveKey, agent, workspaceDir)
}
...
func (e *Engine) cmdSwitch(p Platform, msg *Message, args []string) {
	...
	if sessions.AgentSessionOwnedByOtherKey(msg.SessionKey, matched.ID) {
		e.reply(p, msg.ReplyCtx, e.i18n.T(MsgSwitchOwnedByOtherConversation))
		return
	}
	...
	if inPlace {
		switched, switchErr := sessions.SwitchToAgentSessionWithBack(msg.SessionKey, matched.ID, agent.Name(), matched.Summary)
		if switchErr != nil {
		...
		e.cleanupInteractiveState(interactiveKey)
		slog.Info("cmdSwitch: cleanup done", "session_key", msg.SessionKey)
		if sessions.SwitchToAgentSession(msg.SessionKey, matched.ID, agent.Name(), matched.Summary) == nil {
			e.reply(p, msg.ReplyCtx, e.i18n.T(MsgSwitchOwnedByOtherConversation))
			return
		}
	}
...
func (e *Engine) cmdBack(p Platform, msg *Message) {
	forker, ok := p.(InPlaceConversationForker)
	if !ok || !forker.SupportsInPlaceConversationFork() {
		e.reply(p, msg.ReplyCtx, e.i18n.T(MsgBackUnsupported))
		return
	}
	if msg.inPlaceTransition == nil {
		transition, owner := e.beginOrQueueInPlaceTransition(p, msg)
	...
	target, err := sessions.BackSession(msg.SessionKey)
	if errors.Is(err, ErrNoPreviousSession) {
		e.reply(p, msg.ReplyCtx, e.i18n.T(MsgBackEmpty))
		return
	}
	if err != nil {
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgError, err))
	...
	if strings.TrimSpace(displayName) == "" {
		displayName = target.GetAgentSessionID()
	}
	e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgBackSuccess, displayName))
}
...
func (e *Engine) cmdCancel(p Platform, msg *Message) {
	...
	// Create a replacement session under the same chat key.
...
func (e *Engine) SendToSessionWithOptions(sessionKey, message string, images []ImageAttachment, files []FileAttachment, opts SendOptions) error {
	...
	if message != "" {
		...
		outgoingMessage := e.decorateSessionReply(p, replyCtx, message)
		...
		if len(opts.AtUsers) > 0 || opts.AtAll {
			if atSender, ok := p.(AtMentionSender); ok {
				if err := atSender.ReplyWithAt(e.ctx, replyCtx, outgoingMessage, opts.AtUsers, opts.AtAll); err != nil {
					return err
				}
			} else {
				if err := p.Send(e.ctx, replyCtx, outgoingMessage); err != nil {
					return err
				}
			}
		} else {
...
func (e *Engine) SendToSessionInWorkDir(sessionKey, message string, images []ImageAttachment, files []FileAttachment, workDir string, atUsers []string, atAll bool) error {
	...
	if message != "" {
		...
		outgoingMessage := e.decorateSessionReply(target.platform, target.replyCtx, message)
		if len(atUsers) > 0 || atAll {
			if atSender, ok := target.platform.(AtMentionSender); ok {
				if err := atSender.ReplyWithAt(e.ctx, target.replyCtx, outgoingMessage, atUsers, atAll); err != nil {
					return err
				}
			} else if err := target.platform.Send(e.ctx, target.replyCtx, outgoingMessage); err != nil {
				return err
			}
		} else {
			if err := target.platform.Send(e.ctx, target.replyCtx, outgoingMessage); err != nil {
				return err
			}
		}
		if target.state != nil {
...
func (e *Engine) sendAlreadyRenderedWithError(p Platform, replyCtx any, content string) error {
	content = e.decorateSessionReply(p, replyCtx, content)
	start := time.Now()
...
func (e *Engine) decorateSessionReply(p Platform, replyCtx any, content string) string {
	prefixer, ok := p.(SessionReplyPrefixer)
	if !ok || strings.TrimSpace(content) == "" {
		return content
	}
	sessionKey := strings.TrimSpace(prefixer.ReplySessionKey(replyCtx))
	if sessionKey == "" {
		return content
	}
	...
	if sessions != nil {
		if activeID := sessions.ActiveSessionID(sessionKey); activeID != "" {
			if session := sessions.FindByID(activeID); session != nil {
				agentID := session.GetAgentSessionID()
				sessionName = sessions.GetSessionName(agentID)
				if sessionName == "" {
	...
	return prefixer.FormatSessionReply(sessionName, content)
}
...
func (e *Engine) executeCardAction(cmd, args, sessionKey string) {
	...
	case "/switch":
		...
		if sessions.AgentSessionOwnedByOtherKey(sessionKey, matched.ID) {
			return
		}
		e.cleanupInteractiveState(interactiveKey)
...
func (e *Engine) HandleRelay(ctx context.Context, fromProject, sourceSessionKey, message string) (string, error) {
	...
	if err != nil {
		...
		if session.GetAgentSessionID() != "" && !errors.Is(err, ErrSessionContextPreservationRequired) {
