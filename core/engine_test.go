...
type desktopSyncAgent struct {
	stubAgent
	mu     sync.Mutex
	events map[string][]ExternalConversationEvent
}

func (a *desktopSyncAgent) PollExternalConversation(_ context.Context, sessionID string) ([]ExternalConversationEvent, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
type conversationNameFormattingAgent struct {
	*controllableAgent
	formattedName   string
	formattedInputs []string
}

func (a *conversationNameFormattingAgent) FormatConversationName(name string) string {
	a.formattedInputs = append(a.formattedInputs, name)
	if a.formattedName != "" {
...
type sourceAwareConversationNameFormattingAgent struct {
	*controllableAgent
	sourceCalls []string
	legacyCalls []string
}

func (a *sourceAwareConversationNameFormattingAgent) FormatConversationNameForPlatform(platformName, name string) string {
	a.sourceCalls = append(a.sourceCalls, platformName+":"+name)
	return "[source] " + strings.TrimSpace(name)
}

func (a *sourceAwareConversationNameFormattingAgent) FormatConversationName(name string) string {
	a.legacyCalls = append(a.legacyCalls, name)
	return "[legacy] " + strings.TrimSpace(name)
}

func (p *spawningPlatform) SpawnConversation(_ context.Context, req ConversationSpawnRequest) (SpawnedConversation, error) {
	p.requests = append(p.requests, req)
	return p.spawned, p.spawnErr
}

type desktopSyncPlatform struct {
	stubPlatformEngine
	mu      sync.Mutex
	sentCtx []any
}

func (p *desktopSyncPlatform) ExternalConversationRelayEnabled() bool { return true }

func (p *desktopSyncPlatform) ReconstructReplyCtx(sessionKey string) (any, error) {
	return "reconstructed:" + sessionKey, nil
}

func (p *desktopSyncPlatform) Send(_ context.Context, replyCtx any, content string) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func TestDesktopLiveSyncRelaysMappedAppTurnToFeishu(t *testing.T) {
	agent := &desktopSyncAgent{events: map[string][]ExternalConversationEvent{
		"thread-1": {
			{SessionID: "thread-1", Role: "user", Content: "App 消息"},
			{SessionID: "thread-1", Role: "assistant", Content: "App 回复"},
		},
	}}
	p := &desktopSyncPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)
	s := e.sessions.NewSession("feishu:chat-1:user-1", "test")
	...
	if got := p.sentCtx; !reflect.DeepEqual(got, []any{"reconstructed:feishu:chat-1:user-1", "reconstructed:feishu:chat-1:user-1"}) {
		t.Fatalf("reply contexts = %#v", got)
	}
}

type routeSwitchingDesktopPoller struct {
	sessions   *SessionManager
	sessionKey string
}

func (p *routeSwitchingDesktopPoller) PollExternalConversation(_ context.Context, sessionID string) ([]ExternalConversationEvent, error) {
	newSession := p.sessions.NewSession(p.sessionKey, "new")
	newSession.SetAgentSessionID("thread-new", "codex")
	return []ExternalConversationEvent{{SessionID: sessionID, Role: "user", Content: "stale route message"}}, nil
}

func TestDesktopLiveSyncRechecksRouteBeforeSending(t *testing.T) {
	p := &desktopSyncPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", &desktopSyncAgent{}, []Platform{p}, "", LangChinese)
	sessionKey := "feishu:chat-1:user-1"
	oldSession := e.sessions.NewSession(sessionKey, "old")
	oldSession.SetAgentSessionID("thread-old", "codex")
	if err := e.Start(); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = e.Stop() })

	e.pollDesktopLiveSync(context.Background(), &routeSwitchingDesktopPoller{
		sessions:   e.sessions,
	...
	defer p.mu.Unlock()
	if len(p.sent) != 0 {
		t.Fatalf("stale route events were sent after active session changed: %#v", p.sent)
	}
}
...
func TestCmdNew_SpawnsIndependentConversationAndPreservesSource(t *testing.T) {
	p := &spawningPlatform{
	...
	newAgentSession := newControllableSession("thread-b")
	agent := &controllableAgent{nextSession: newAgentSession}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)

	const sourceKey = "feishu:chat-a:user-1"
	sourceSession := e.sessions.GetOrCreateActive(sourceKey)
	sourceSession.SetAgentSessionID("thread-a", "codex")
	sourceSession.AddHistory("user", "A 群历史消息")
	e.sessions.Save()
	sourceSessionID := e.sessions.ActiveSessionID(sourceKey)
	sourceHistory := append([]HistoryEntry(nil), sourceSession.History...)
	sourceLive := newControllableSession("thread-a")
	sourceState := &interactiveState{agentSession: sourceLive, platform: p, replyCtx: "ctx-a", agent: agent}
	e.interactiveStates[sourceKey] = sourceState
	...
	wantRequest := ConversationSpawnRequest{Name: "新项目", UserID: "user-1", MessageID: "message-1"}
	if !reflect.DeepEqual(p.requests[0], wantRequest) {
		t.Fatalf("SpawnConversation request = %#v, want %#v", p.requests[0], wantRequest)
	}
	if got := e.sessions.ActiveSessionID(sourceKey); got != sourceSessionID {
		t.Fatalf("source active session = %q, want unchanged %q", got, sourceSessionID)
	}
	if got := sourceSession.GetAgentSessionID(); got != "thread-a" {
		t.Fatalf("source agent session = %q, want thread-a", got)
	}
	if !reflect.DeepEqual(sourceSession.History, sourceHistory) {
		t.Fatalf("source history changed: %#v", sourceSession.History)
	}
	if !sourceLive.Alive() {
		t.Fatal("source live agent session was closed")
	}
	if got := e.interactiveStates[sourceKey]; got != sourceState {
		t.Fatal("source interactive state was replaced")
	}

	targetSession := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if got := targetSession.GetAgentSessionID(); got != "thread-b" {
		t.Fatalf("target agent session = %q, want thread-b", got)
	}
	if got := targetSession.GetName(); got != "新项目" {
		t.Fatalf("target session name = %q, want 新项目", got)
	}
	if got := e.interactiveStates["feishu:chat-b:user-1"]; got == nil || got.agentSession != newAgentSession {
		t.Fatal("target interactive state was not created")
	}
}

func TestCmdNew_AppliesAgentConversationNameFormatter(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	agent := &conversationNameFormattingAgent{
		controllableAgent: &controllableAgent{nextSession: session},
	}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)
	...
	e.cmdNew(p, &Message{
		...
		UserID:     "user-1",
		MessageID:  "message-prefixed-name",
		ChatName:   "A群",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})
	...
	if got := p.requests[0].Name; got != "[Codex] 新项目" {
		t.Fatalf("spawn name = %q, want [Codex] 新项目", got)
	}
	targetSession := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if got := targetSession.GetName(); got != "[Codex] 新项目" {
		t.Fatalf("target session name = %q, want [Codex] 新项目", got)
	}
	if got := session.name; got != "[Codex] 新项目" {
		t.Fatalf("Codex session name = %q, want [Codex] 新项目", got)
	}
}

func TestCmdNew_PrefersSourceAwareConversationNameFormatter(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	agent := &sourceAwareConversationNameFormattingAgent{
		controllableAgent: &controllableAgent{nextSession: newControllableSession("thread-b")},
	}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)
	...
	e.cmdNew(p, &Message{
		SessionKey: "feishu:chat-a:user-1",
		UserID:     "user-1",
		MessageID:  "message-source-aware-name",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})

	if len(p.requests) != 1 || p.requests[0].Name != "[source] 新项目" {
		t.Fatalf("spawn requests = %#v, want one source-aware name", p.requests)
	}
	if !reflect.DeepEqual(agent.sourceCalls, []string{"feishu:新项目"}) {
		t.Fatalf("source-aware calls = %#v, want [feishu:新项目]", agent.sourceCalls)
	}
	if len(agent.legacyCalls) != 0 {
		t.Fatalf("legacy formatter calls = %#v, want none", agent.legacyCalls)
	}
}

func TestCmdNew_FormatsDerivedDefaultConversationName(t *testing.T) {
	tests := []struct {
		name      string
		chatName  string
		wantInput string
		...
		{name: "普通群名", chatName: "精选文章收集", wantInput: "精选文章收集-新会话", want: "[Codex] 精选文章收集-新会话"},
		{name: "已有前缀", chatName: "[CODEX]   精选文章收集", wantInput: "[CODEX]   精选文章收集-新会话", want: "[Codex] 精选文章收集-新会话"},
		{name: "无群名", chatName: "", wantInput: "新会话", want: "[Codex] 新会话"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e.cmdNew(p, &Message{
				SessionKey: "feishu:chat-a:user-1",
				UserID:     "user-1",
				MessageID:  "message-default-prefixed-name",
				ChatName:   tt.chatName,
				ReplyCtx:   "ctx-a",
			}, nil)
			...
			if !reflect.DeepEqual(agent.formattedInputs, []string{tt.wantInput}) {
				t.Fatalf("formatter inputs = %#v, want [%q]", agent.formattedInputs, tt.wantInput)
			}
			if got := p.requests[0].Name; got != tt.want {
				t.Fatalf("spawn name = %q, want %q", got, tt.want)
			}
			targetSession := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
			if got := targetSession.GetName(); got != tt.want {
				t.Fatalf("target session name = %q, want %q", got, tt.want)
			}
			if got := session.name; got != tt.want {
				t.Fatalf("Codex session name = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestCmdNew_UsesSourceGroupNameByDefault(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned:            SpawnedConversation{SessionKey: "feishu:chat-b:user-1", ReplyCtx: "ctx-b"},
	}
	e := NewEngine("test", &controllableAgent{nextSession: newControllableSession("thread-b")}, []Platform{p}, "", LangChinese)
	...
	if len(p.requests) != 1 || p.requests[0].Name != "精选文章收集-新会话" {
		t.Fatalf("spawn requests = %#v, want derived group name", p.requests)
	}
}

func TestCmdNew_UnsupportedPlatformPreservesSource(t *testing.T) {
	p := &stubPlatformEngine{n: "plain"}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangChinese)
	const sourceKey = "plain:chat-a:user-1"
	sourceSession := e.sessions.GetOrCreateActive(sourceKey)
	sourceSession.SetAgentSessionID("thread-a", "codex")
	sourceID := e.sessions.ActiveSessionID(sourceKey)

	e.cmdNew(p, &Message{SessionKey: sourceKey, ReplyCtx: "ctx-a"}, nil)

	if got := e.sessions.ActiveSessionID(sourceKey); got != sourceID {
		t.Fatalf("source active session = %q, want unchanged %q", got, sourceID)
	}
	if got := sourceSession.GetAgentSessionID(); got != "thread-a" {
		t.Fatalf("source agent session = %q, want thread-a", got)
	}
...
func TestCmdNew_SpawnFailurePreservesSource(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawnErr:           errors.New("create chat failed"),
	}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangChinese)
	const sourceKey = "feishu:chat-a:user-1"
	sourceSession := e.sessions.GetOrCreateActive(sourceKey)
	sourceSession.SetAgentSessionID("thread-a", "codex")
	sourceSession.AddHistory("user", "keep me")
	sourceID := e.sessions.ActiveSessionID(sourceKey)
	...
	if got := len(sourceSession.History); got != 1 {
		t.Fatalf("source history length = %d, want 1", got)
	}
}

func TestCmdNew_AgentInitFailureKeepsSourceAndTargetForRetry(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	if got := target.GetAgentSessionID(); got != "" {
		t.Fatalf("target agent session = %q, want empty for retry", got)
	}
	state := e.interactiveStates["feishu:chat-b:user-1"]
	if state == nil || state.agentSession != nil {
		t.Fatalf("target retry state = %#v, want placeholder with nil agent session", state)
	}
	if got := strings.Join(p.getSent(), "\n"); !strings.Contains(got, "发送消息重试") {
		t.Fatalf("failure replies = %q, want retry guidance", got)
	}
}

func TestCmdNew_EmptyAgentSessionIDIsNotReportedAsCreated(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	e := NewEngine("test", &controllableAgent{nextSession: newCodexLikeSession("thread-later")}, []Platform{p}, "", LangChinese)

	e.cmdNew(p, &Message{
		SessionKey: "feishu:chat-a:user-1",
		UserID:     "user-1",
		MessageID:  "message-empty-id",
		ChatName:   "A群",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})
	...
	target := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if got := target.GetAgentSessionID(); got != "" {
		t.Fatalf("target agent session = %q, want empty before the first message", got)
	}
	replies := strings.Join(p.getSent(), "\n")
	if !strings.Contains(replies, "初始化失败") {
		t.Fatalf("replies = %q, want initialization failure", replies)
	}
	if strings.Contains(replies, "已一对一绑定") {
		t.Fatalf("replies = %q, must not claim the Codex session was created", replies)
	}
}

func TestCmdNew_NameSyncFailureDoesNotClaimFullSuccess(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	session := &namingAgentSession{
		controllableAgentSession: newControllableSession("thread-b"),
		err:                      errors.New("name RPC failed"),
	}
	e := NewEngine("test", &controllableAgent{nextSession: session}, []Platform{p}, "", LangChinese)

	e.cmdNew(p, &Message{
		SessionKey: "feishu:chat-a:user-1",
		UserID:     "user-1",
		MessageID:  "message-name-failure",
		ChatName:   "A群",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})

	if got := e.sessions.GetOrCreateActive("feishu:chat-b:user-1").GetAgentSessionID(); got != "thread-b" {
		t.Fatalf("target agent session = %q, want thread-b", got)
	}
	replies := strings.Join(p.getSent(), "\n")
	if !strings.Contains(replies, "名称同步失败") {
		t.Fatalf("replies = %q, want name sync warning", replies)
	}
	if strings.Contains(replies, "已一对一绑定") {
		t.Fatalf("replies = %q, must not claim full success", replies)
	}
}

func TestCmdNew_PrimesVisibilityBeforeNameSync(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	if !reflect.DeepEqual(session.primedNames, []string{"新项目"}) {
		t.Fatalf("primed names = %#v, want [新项目]", session.primedNames)
	}
	if !reflect.DeepEqual(session.names, []string{"新项目"}) {
		t.Fatalf("synced names = %#v, want [新项目]", session.names)
	}
	if got := strings.Join(p.getSent(), "\n"); !strings.Contains(got, "已一对一绑定") {
		t.Fatalf("replies = %q, want success after name sync", got)
	}
}

func TestCmdNew_DoesNotClaimSuccessWhenVisibilityPrimeFails(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	session := &primingAgentSession{
		namingAgentSession: &namingAgentSession{controllableAgentSession: newControllableSession("thread-b")},
		primeErr:           errors.New("visibility initialization failed"),
	}
	e := NewEngine("test", &controllableAgent{nextSession: session}, []Platform{p}, "", LangChinese)
	...
	e.cmdNew(p, &Message{
		SessionKey: "feishu:chat-a:user-1",
		UserID:     "user-1",
		MessageID:  "message-prime-failure",
		ChatName:   "A群",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})
	...
	if replies := strings.Join(p.getSent(), "\n"); strings.Contains(replies, "已一对一绑定") {
		t.Fatalf("replies = %q, must not claim success after visibility failure", replies)
	}
}

func TestCmdNew_QueuesFirstTargetMessageDuringVisibilityPrime(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	session := &primingAgentSession{
		namingAgentSession: &namingAgentSession{controllableAgentSession: baseSession},
		primeStarted:       primeStarted,
	...
	go func() {
		e.cmdNew(p, &Message{
			SessionKey: "feishu:chat-a:user-1",
			UserID:     "user-1",
			MessageID:  "message-network-independent-new",
			ChatName:   "A群",
			ReplyCtx:   "ctx-a",
		}, []string{"新项目"})
		close(newDone)
	...
	case <-time.After(time.Second):
		close(primeRelease)
		t.Fatal("/new did not start the visibility prime")
	}

	target := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if !target.Busy() {
		close(primeRelease)
		t.Fatal("target session was released before visibility prime completed")
	}
	e.ReceiveMessage(p, &Message{
		...
		UserID:     "user-1",
		UserName:   "用户",
		MessageID:  "first-target-message-after-new",
		ChatName:   "新项目",
		Content:    "第一条真实消息",
		ReplyCtx:   "ctx-first",
	})
	...
	case prompt := <-baseSession.sendPrompts:
		close(primeRelease)
		t.Fatalf("first message started before visibility prime completed: %q", prompt)
	...
	case <-newDone:
	case <-time.After(time.Second):
		t.Fatal("/new did not finish after visibility prime completed")
	}

	select {
	case prompt := <-baseSession.sendPrompts:
		if !strings.Contains(prompt, "第一条真实消息") {
			t.Fatalf("first prompt = %q, want first target message", prompt)
		}
	case <-time.After(time.Second):
		t.Fatal("first target message did not reach Codex immediately")
	}
	baseSession.events <- Event{Type: EventText, Content: "第一条真实回复"}
	...
	t.Fatalf("first target message did not finish cleanly; replies=%q busy=%v", strings.Join(p.getSent(), "\n"), target.Busy())
}

func TestCmdNew_DrainsFirstTargetMessageQueuedDuringNameSync(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	go func() {
		e.cmdNew(p, &Message{
			SessionKey: sourceKey,
			UserID:     "user-1",
			MessageID:  "message-name-sync-race",
			ChatName:   "A群",
			ReplyCtx:   "ctx-a",
		}, []string{"新项目"})
		close(newDone)
	...
	target := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if target.TryLock() {
		t.Fatal("target session was not busy during name sync")
	}
	e.ReceiveMessage(p, &Message{
		Platform:   "feishu",
		SessionKey: "feishu:chat-b:user-1",
		UserID:     "user-1",
		UserName:   "用户",
		MessageID:  "first-target-message-during-name-sync",
		ChatName:   "新项目",
		Content:    "名称同步期间的第一条消息",
		ReplyCtx:   "ctx-first",
	})

	select {
	case prompt := <-baseSession.sendPrompts:
		t.Fatalf("first target message bypassed the initialization lock: %q", prompt)
	case <-time.After(50 * time.Millisecond):
	}
	...
	case <-newDone:
	case <-time.After(time.Second):
		t.Fatal("/new did not finish after name sync was released")
	}

	select {
	case prompt := <-baseSession.sendPrompts:
		if !strings.Contains(prompt, "名称同步期间的第一条消息") {
			t.Fatalf("queued prompt = %q, want first target message", prompt)
		}
	case <-time.After(time.Second):
		t.Fatal("queued first target message was not sent after name sync")
	}
	baseSession.events <- Event{Type: EventText, Content: "并发首条消息回复"}
	baseSession.events <- Event{Type: EventResult, SessionID: "thread-b", Done: true}

	deadline := time.Now().Add(time.Second)
	for time.Now().Before(deadline) {
		if strings.Contains(strings.Join(p.getSent(), "\n"), "并发首条消息回复") && !target.Busy() {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	if target.Busy() || !strings.Contains(strings.Join(p.getSent(), "\n"), "并发首条消息回复") {
		t.Fatalf("queued first message did not finish cleanly; replies=%q busy=%v", strings.Join(p.getSent(), "\n"), target.Busy())
	}
	if got := target.GetHistory(0); len(got) != 2 || got[0].Content != "名称同步期间的第一条消息" {
		t.Fatalf("target history = %#v, want queued first turn", got)
	}
	if got := e.sessions.GetOrCreateActive(sourceKey).ID; got != sourceID {
		t.Fatalf("source session changed: got %s, want %s", got, sourceID)
	}
	if got := source.GetHistory(0); !reflect.DeepEqual(got, sourceHistory) {
		t.Fatalf("source history changed: got %#v, want %#v", got, sourceHistory)
	}
}

func TestCmdNew_BusyTargetDoesNotClaimFullSuccess(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	target := e.sessions.GetOrCreateActive("feishu:chat-b:user-1")
	if !target.TryLock() {
		t.Fatal("failed to pre-lock target session")
	}
	defer target.UnlockWithoutUpdate()

	e.cmdNew(p, &Message{
		SessionKey: "feishu:chat-a:user-1",
		UserID:     "user-1",
		MessageID:  "message-target-busy",
		ChatName:   "A群",
		ReplyCtx:   "ctx-a",
	}, []string{"新项目"})

	if got := target.GetName(); got != "新项目" {
		t.Fatalf("target session name = %q, want 新项目", got)
	}
	replies := strings.Join(p.getSent(), "\n")
	if !strings.Contains(replies, "正在初始化") {
		t.Fatalf("replies = %q, want initialization-in-progress notice", replies)
	}
	if strings.Contains(replies, "已一对一绑定") {
		t.Fatalf("replies = %q, must not claim full success while target is busy", replies)
	}
}

func TestCmdNew_RepeatedDeliveryReusesTargetSession(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	firstID := e.sessions.ActiveSessionID("feishu:chat-b:user-1")
	e.cmdNew(p, msg, []string{"新项目"})
	secondID := e.sessions.ActiveSessionID("feishu:chat-b:user-1")

	if firstID == "" || secondID != firstID {
		t.Fatalf("target session IDs = %q then %q, want same non-empty ID", firstID, secondID)
	}
...
func TestCmdNew_InjectsTargetSessionKey(t *testing.T) {
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	agent := &sessionEnvRecordingAgent{session: newControllableSession("thread-b")}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)
	...
	if got := agent.EnvValue("CC_SESSION_KEY"); got != "feishu:chat-b:user-1" {
		t.Fatalf("CC_SESSION_KEY = %q, want target session key", got)
	}
}

func TestCmdNew_MultiWorkspaceBindsTargetToSourceWorkspace(t *testing.T) {
	baseDir := t.TempDir()
	workspaceDir := filepath.Join(baseDir, "workspace-a")
	if err := os.MkdirAll(workspaceDir, 0o755); err != nil {
		t.Fatal(err)
	}
	normalizedWorkspace := normalizeWorkspacePath(workspaceDir)
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned: SpawnedConversation{
			SessionKey: "feishu:chat-b:user-1",
	...
	e.workspaceBindings.Bind("project:test", "feishu:chat-a", "A群", normalizedWorkspace)

	workspaceAgent := &controllableAgent{nextSession: newControllableSession("thread-b")}
	workspaceSessions := NewSessionManager("")
	workspace := e.workspacePool.GetOrCreate(normalizedWorkspace)
	...
	targetMsg := &Message{Platform: "feishu", SessionKey: "feishu:chat-b:user-1"}
	resolvedAgent, resolvedSessions, interactiveKey, resolvedWorkspace, err := e.commandContextWithWorkspace(p, targetMsg)
	if err != nil {
		t.Fatalf("resolve target workspace: %v", err)
	}
	if resolvedAgent != workspaceAgent || resolvedSessions != workspaceSessions {
		t.Fatal("target group did not resolve to the source workspace agent and sessions")
	}
	if resolvedWorkspace != normalizedWorkspace {
		t.Fatalf("target workspace = %q, want %q", resolvedWorkspace, normalizedWorkspace)
	}
	wantInteractiveKey := normalizedWorkspace + ":feishu:chat-b:user-1"
	if interactiveKey != wantInteractiveKey {
		t.Fatalf("target interactive key = %q, want %q", interactiveKey, wantInteractiveKey)
	}
	if got := resolvedSessions.GetOrCreateActive("feishu:chat-b:user-1").GetAgentSessionID(); got != "thread-b" {
		t.Fatalf("target agent session = %q, want thread-b", got)
	}
}

func TestHelpCardNewDispatchesCommandInsteadOfMutatingSessionDirectly(t *testing.T) {
	groups := helpCardGroups()
	for _, group := range groups {
		for _, item := range group.items {
			if item.command == "/new" {
				if item.action != "cmd:/new" {
					t.Fatalf("/new help action = %q, want cmd:/new", item.action)
				}
				return
			}
		}
	}
	t.Fatal("/new help action not found")
}
...
type controllableAgentSession struct {
	...
	usageErr        error
	sendPrompts     chan string
}

type namingAgentSession struct {
...
func (s *primingAgentSession) PrimeSession(name string) error {
	s.primedNames = append(s.primedNames, name)
	if s.primeStarted != nil {
		close(s.primeStarted)
	}
	if s.primeRelease != nil {
		<-s.primeRelease
	}
	return s.primeErr
}
...
func (s *namingAgentSession) ArchiveSession(sessionID string) error {
	s.archivedIDs = append(s.archivedIDs, sessionID)
	return s.archiveErr
}

func TestSyncAgentSessionName(t *testing.T) {
	s := &namingAgentSession{controllableAgentSession: newControllableSession("thread-1")}
	if err := syncAgentSessionName(s, "  [Codex] 项目群  "); err != nil {
		t.Fatalf("syncAgentSessionName returned error: %v", err)
	}
	if s.name != "[Codex] 项目群" {
		t.Fatalf("synced name = %q", s.name)
	}
...
func TestCmdNameWeComRestoresPersistedThreadWhenNoLiveState(t *testing.T) {
	p := &stubPlatformEngine{n: "wecom"}
	const sessionKey = "wecom:g:group-1"
	...
	if got := startedWith; got != targetID {
		t.Fatalf("StartSession target = %q, want existing thread %q", got, targetID)
	}
	...
	default:
		t.Fatal("restored agent session was not closed")
	}
	...
	if sent := p.getSent(); len(sent) != 1 || !strings.Contains(sent[0], "已命名") {
		t.Fatalf("reply = %#v, want success", sent)
	}
}

func TestCmdNameWeComReportsRestoreRenameFailureWhenNoLiveState(t *testing.T) {
	p := &stubPlatformEngine{n: "wecom"}
	const sessionKey = "wecom:g:group-1"
	const targetID = "thread-wecom"
	const want = "[企业微信-Codex] 企业微信A"
	...
	restored := &namingAgentSession{
		...
		err:                      errors.New("desktop rename failed"),
	}
	agent := &wecomConversationNameAgent{controllableAgent: &controllableAgent{
		startSessionFn: func(_ context.Context, sessionID string) (AgentSession, error) {
	...
	if got := e.sessions.GetSessionName(targetID); got != want {
		t.Fatalf("persisted name = %q, want %q despite restore failure", got, want)
	}
	if got := session.GetAgentSessionID(); got != targetID {
		t.Fatalf("bound session ID = %q, want unchanged %q", got, targetID)
	}
	...
	case <-restored.closed:
	default:
		t.Fatal("restored agent session was not closed after rename failure")
	}
	if sent := p.getSent(); len(sent) != 1 || !strings.Contains(sent[0], "已保存") || !strings.Contains(sent[0], "即时同步失败") {
...
func TestProcessInteractiveMessageWith_InPlacePlatformKeepsLogicalSessionNameAfterOperation(t *testing.T) {
	...
	e := NewEngine("test", &controllableAgent{nextSession: agentSession}, []Platform{p}, "", LangChinese)

	const sessionKey = "weixin:dm:peer"
	session := e.sessions.NewSession(sessionKey, "[Codex] 微信逻辑会话实机验收-0722")
	session.SetAgentSessionID("thread-new", e.agent.Name())
	if !session.TryLock() {
		t.Fatal("expected session lock")
	}
	...
	e.processInteractiveMessageWith(p, &Message{
		SessionKey: sessionKey,
		UserID:     "peer",
		Content:    "微信首条路由验收-0722",
		ChatName:   "[Codex] 微信",
		ReplyCtx:   "ctx",
	}, session, e.agent, e.sessions, sessionKey, "", sessionKey)

	want := []string{
		"[Codex] 微信逻辑会话实机验收-0722",
		"[Codex] 微信逻辑会话实机验收-0722",
	}
	if !reflect.DeepEqual(agentSession.names, want) {
		t.Fatalf("synced names = %#v, want logical session name before and after turn %#v", agentSession.names, want)
	}
}

func TestProcessInteractiveMessageWith_ConversationSpawnerKeepsExternalChatName(t *testing.T) {
	p := &externalFirstNewPlatform{spawningPlatform: &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
	}}
	agentSession := &namingAgentSession{controllableAgentSession: newControllableSession("thread-external")}
	e := NewEngine("test", &controllableAgent{nextSession: agentSession}, []Platform{p}, "", LangChinese)

	const sessionKey = "feishu:chat:peer"
	session := e.sessions.NewSession(sessionKey, "[Codex] 旧逻辑名")
	session.SetAgentSessionID("thread-external", e.agent.Name())
	if !session.TryLock() {
		t.Fatal("expected session lock")
	}
	e.interactiveStates[sessionKey] = &interactiveState{
	...
	want := []string{"[Codex] 飞书群名", "[Codex] 飞书群名"}
	if !reflect.DeepEqual(agentSession.names, want) {
		t.Fatalf("synced names = %#v, want external chat name before and after turn %#v", agentSession.names, want)
	}
}

func TestArchivePendingAgentSession(t *testing.T) {
	s := &namingAgentSession{controllableAgentSession: newControllableSession("thread-new")}
	ccSession := &Session{PendingArchiveAgentSessionID: "thread-old"}

	archived, err := archivePendingAgentSession(s, ccSession)
	if err != nil {
		t.Fatalf("archivePendingAgentSession returned error: %v", err)
	}
	if !archived {
		t.Fatal("archivePendingAgentSession archived = false, want true")
	}
	if !reflect.DeepEqual(s.archivedIDs, []string{"thread-old"}) {
		t.Fatalf("archived IDs = %#v", s.archivedIDs)
	}
	if got := ccSession.GetPendingArchiveAgentSessionID(); got != "" {
		t.Fatalf("pending archive ID = %q, want empty", got)
	}

	ccSession.SetPendingArchiveAgentSessionID("thread-retry")
	s.archiveErr = errors.New("archive failed")
	archived, err = archivePendingAgentSession(s, ccSession)
	if err == nil {
		t.Fatal("archivePendingAgentSession error = nil, want failure")
	}
	if archived {
		t.Fatal("archivePendingAgentSession archived = true after failure")
	}
	if got := ccSession.GetPendingArchiveAgentSessionID(); got != "thread-retry" {
		t.Fatalf("pending archive ID = %q, want retry target preserved", got)
	}

	ccSession.ClearPendingArchiveAgentSessionID("thread-retry")
	archived, err = archivePendingAgentSession(s, ccSession)
	if err != nil {
		t.Fatalf("archivePendingAgentSession no-op returned error: %v", err)
	}
	if archived {
		t.Fatal("archivePendingAgentSession archived = true without a pending archive")
	}
}
...
func (s *controllableAgentSession) Send(prompt string, _ []ImageAttachment, _ []FileAttachment) error {
	if s.sendPrompts != nil {
		s.sendPrompts <- prompt
	}
	return nil
}
...
func TestResumeContextPreservationFailureDoesNotFallbackToFreshSession(t *testing.T) {
	agent := &stubStartSessionAgent{
		failIDs: map[string]error{
			"archived-session-id": fmt.Errorf("%w: unarchive failed", ErrSessionContextPreservationRequired),
		},
	}
	...
	session := e.sessions.GetOrCreateActive("test:user1")
	session.SetAgentSessionID("archived-session-id", "stub")

	p := &stubPlatformEngine{n: "test"}
	state := e.getOrCreateInteractiveStateWith("test:user1", p, "ctx", session, e.sessions, nil, "")

	if state.agentSession != nil {
		t.Fatal("context-preserving resume failure must not create a fresh session")
	}
	agent.mu.Lock()
	calls := append([]string{}, agent.calls...)
	agent.mu.Unlock()
	if !reflect.DeepEqual(calls, []string{"archived-session-id"}) {
		t.Fatalf("StartSession calls = %#v, want only archived session resume", calls)
	}
	if got := session.GetAgentSessionID(); got != "archived-session-id" {
...
func TestExecuteCardAction_LegacyNewDoesNotMutateSession(t *testing.T) {
	p := &stubPlatformEngine{n: "test"}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)
	key := "test:user1"

	e.interactiveMu.Lock()
	state := &interactiveState{agentSession: newControllableSession("old")}
	e.interactiveStates[key] = state
	e.interactiveMu.Unlock()
	e.sessions.GetOrCreateActive(key)
	sessionID := e.sessions.ActiveSessionID(key)

	e.executeCardAction("/new", "", key)

	e.interactiveMu.Lock()
	gotState := e.interactiveStates[key]
	e.interactiveMu.Unlock()

	if gotState != state {
		t.Error("legacy act:/new should not mutate the interactive state")
	}
	if got := e.sessions.ActiveSessionID(key); got != sessionID {
		t.Fatalf("legacy act:/new changed active session from %q to %q", sessionID, got)
	}
}
...
func TestCmdSwitch_RejectsSessionOwnedByAnotherFeishuGroup(t *testing.T) {
	agent := &controllableAgent{listFn: func() ([]AgentSessionInfo, error) {
		return []AgentSessionInfo{{ID: "thread-b", Summary: "B task"}}, nil
	}}
	p := &stubPlatformEngine{n: "feishu"}
	e := NewEngine("test", agent, []Platform{p}, "", LangChinese)
	aKey := "feishu:chat-a:user-1"
	bKey := "feishu:chat-b:user-1"
	a := e.sessions.NewSession(aKey, "A")
	a.SetAgentSessionID("thread-a", "codex")
	b := e.sessions.NewSession(bKey, "B")
	b.SetAgentSessionID("thread-b", "codex")
	aBefore := e.sessions.ActiveSessionID(aKey)

	e.cmdSwitch(p, &Message{SessionKey: aKey, ReplyCtx: "ctx-a"}, []string{"thread-b"})

	if got := e.sessions.ActiveSessionID(aKey); got != aBefore {
		t.Fatalf("A active session changed from %q to %q", aBefore, got)
	}
	if replies := strings.Join(p.getSent(), "\n"); !strings.Contains(replies, "已绑定其他群") {
		t.Fatalf("replies = %q, want cross-group ownership rejection", replies)
	}
}
...
func TestCmdList_AllSessionsVisibleAfterRepeatedNew(t *testing.T) {
	...
	targetKey := "feishu:new-chat:user1"
	p := &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "feishu"},
		spawned:            SpawnedConversation{SessionKey: targetKey, ReplyCtx: "new-ctx"},
	}
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	userKey := "test:user1"
...
func TestCmdList_RealWorldLegacyDataFullFlow(t *testing.T) {
	...
	msg := &Message{SessionKey: userKey, UserID: "user1", MessageID: "om-new", ChatName: "旧群", ReplyCtx: "ctx"}
	...
	if got := e.sessions.ActiveSessionID(userKey); got != "s15" {
		t.Fatalf("step2: source active session = %q, want unchanged s15", got)
	}
	newSession := e.sessions.GetOrCreateActive(targetKey)
	if got := newSession.GetName(); got != "我的新会话" {
		t.Fatalf("step2: target name = %q, want 我的新会话", got)
	}

	// The stub agent returns its thread ID immediately during /new.
	newThreadID := newSession.GetAgentSessionID()
	...
	// ── Step 3: verify the new target session name on page 2 ───
	// The newest session is at the end of the list; check page 2.
	p.sent = nil
	e.cmdList(p, &Message{SessionKey: targetKey, ReplyCtx: "new-ctx"}, []string{"2"})
	if len(p.sent) != 1 {
		t.Fatalf("step3: expected 1 reply for page 2, got %d", len(p.sent))
	}
	if !strings.Contains(p.sent[0], "我的新会话") {
		t.Errorf("step3: /list page 2 should display session name '我的新会话' but it's missing:\n%s", p.sent[0])
	}
}
...
func TestSessionName_CodexLikeFlow(t *testing.T) {
	baseSession := newCodexLikeSession("codex-thread-new-001")
	sess := &namingCodexLikeSession{codexLikeSession: baseSession}
	...
	// Step 1: /new "我的新会话"
	e.cmdNew(p, &Message{SessionKey: userKey, UserID: "user1", MessageID: "om-new-1", ReplyCtx: "ctx"}, []string{"我的新会话"})
	...
	if len(sess.archivedIDs) != 0 {
		t.Fatalf("archived IDs = %#v, want source Codex task untouched", sess.archivedIDs)
	}
	if got := initial.GetAgentSessionID(); got != "codex-thread-old" {
		t.Fatalf("source agent session = %q, want codex-thread-old", got)
	}
	...
	p.sent = nil
	e.cmdList(p, &Message{SessionKey: targetKey, ReplyCtx: "ctx"}, nil)
	if len(p.sent) != 1 {
		t.Fatalf("expected 1 reply, got %d", len(p.sent))
	}
	if !strings.Contains(p.sent[0], "我的新会话") {
...
type namingCodexLikeSession struct {
	*codexLikeSession
	names       []string
	archivedIDs []string
}

func (s *namingCodexLikeSession) SetSessionName(name string) error {
...
func (s *namingCodexLikeSession) ArchiveSession(sessionID string) error {
	s.archivedIDs = append(s.archivedIDs, sessionID)
	return nil
}
...
func TestSessionName_ClaudeCodeLikeFlow(t *testing.T) {
	...
	e.cmdNew(p, &Message{SessionKey: userKey, UserID: "user1", MessageID: "om-new-2", ReplyCtx: "ctx"}, []string{"Claude任务"})
...
func TestSessionName_ACPLikeFlow(t *testing.T) {
	...
	// /new with a custom name
	e.cmdNew(p, &Message{SessionKey: userKey, UserID: "user1", MessageID: "om-new-3", ReplyCtx: "ctx"}, []string{"ACP任务"})
