...
func newCUJEnv(t *testing.T) *cujEnv {
	...
	agent := &cujAgent{}
	e := NewEngine("test", agent, []Platform{plat}, "", LangEnglish)
	return &cujEnv{
		t:       t,
		engine:  e,
		plat:    plat,
...
// CUJ-B1 · /new creates a new group with an independent session while the
// source group remains on its original session.
func TestCUJ_B1_NewCreatesIndependentSession(t *testing.T) {
	const (
		sourceKey = "test:chat-a:b1"
		targetKey = "test:chat-b:b1"
	)
	p := &spawningPlatform{
	...
	operations, err := NewNewOperationStore(dir + "/operations.json")
	if err != nil {
		t.Fatal(err)
	}
	e.SetNewOperationStore(operations)
	...
	for len(p.getSent()) < 1 && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	...
	op, ok := operations.Get(NewOperationID("test", sourceKey, "msg-new"))
	if !ok || op.Status != NewOperationStatusCompleted {
		t.Fatalf("durable /new operation did not complete: %+v", op)
	}

	if got := e.sessions.GetOrCreateActive(sourceKey).ID; got != sourceID {
		t.Fatalf("source group session = %s, want unchanged %s", got, sourceID)
	}
	if got := source.GetHistory(0); !reflect.DeepEqual(got, sourceHistory) {
		t.Fatalf("source history changed: %#v, want %#v", got, sourceHistory)
	}
	target := e.sessions.GetOrCreateActive(targetKey)
	if target.ID == sourceID {
		t.Fatalf("target group reused source session %s", sourceID)
	}
	if len(target.GetHistory(0)) != 0 {
		t.Fatalf("target session should start with empty history, got %d entries", len(target.GetHistory(0)))
	}

	// The first real message in the newly created group must start immediately;
	// /new must not hold the target lock for a model-backed visibility turn.
	p.clearSent()
	e.ReceiveMessage(p, &Message{
		SessionKey: targetKey,
		Platform:   "test",
		MessageID:  "msg-target-first",
		UserID:     "b1",
		UserName:   "b1",
		ChatName:   "A group-new conversation",
		Content:    "first target message",
		ReplyCtx:   "ctx-b",
	})
	deadline = time.Now().Add(2 * time.Second)
	for (len(p.getSent()) < 1 || target.Busy()) && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	if len(p.getSent()) < 1 || target.Busy() {
		t.Fatalf("first target message did not complete; replies=%q busy=%v", strings.Join(p.getSent(), "\n"), target.Busy())
	}
	if got := target.GetHistory(0); len(got) != 2 || got[0].Role != "user" || got[0].Content != "first target message" {
		t.Fatalf("target first turn history = %#v, want user message and assistant reply", got)
	}
	if got := e.sessions.GetOrCreateActive(sourceKey).ID; got != sourceID {
		t.Fatalf("source group changed after target message: got %s, want %s", got, sourceID)
	}
	if got := source.GetHistory(0); !reflect.DeepEqual(got, sourceHistory) {
		t.Fatalf("source history changed after target message: %#v, want %#v", got, sourceHistory)
	}
}

// CUJ-B1R · If cc-connect stops after the new group exists but before its
// Codex session starts, the next process resumes the durable operation. The
// source group keeps its history and the target group is not created twice.
func TestCUJ_B1R_NewFinishesAfterProcessRestart(t *testing.T) {
	const (
		sourceKey = "test:chat-a:b1-restart"
		targetKey = "test:chat-b:b1-restart"
	)
	dir := t.TempDir()
	sessionPath := dir + "/sessions.json"
	operationPath := dir + "/operations.json"
	p1 := &reconstructingSpawningPlatform{spawningPlatform: &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "test"},
		spawned:            SpawnedConversation{SessionKey: targetKey, ReplyCtx: "ctx-b"},
	}}
	agent1 := &cujAgent{}
	e1 := NewEngine("test", agent1, []Platform{p1}, sessionPath, LangEnglish)
	e1.newOperationRetryDelay = 100 * time.Millisecond
	operations1, err := NewNewOperationStore(operationPath)
	if err != nil {
		t.Fatal(err)
	}
	e1.SetNewOperationStore(operations1)

	// Action 1: establish real context in A.
	e1.ReceiveMessage(p1, &Message{
		SessionKey: sourceKey, Platform: "test", MessageID: "msg-context",
		UserID: "b1-restart", UserName: "b1-restart", ChatName: "A group",
		Content: "context that must survive", ReplyCtx: "ctx-a",
	})
	deadline := time.Now().Add(2 * time.Second)
	for len(p1.getSent()) < 1 && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	source := e1.sessions.GetOrCreateActive(sourceKey)
	sourceID := source.ID
	sourceHistory := source.GetHistory(0)
	if len(sourceHistory) != 2 {
		t.Fatalf("source history before /new = %#v", sourceHistory)
	}

	// Action 2: /new creates B, then agent startup fails and the process stops.
	agent1.failStartCount = 1
	agent1.failStartErr = errors.New("simulated network interruption")
	p1.clearSent()
	e1.ReceiveMessage(p1, &Message{
		SessionKey: sourceKey, Platform: "test", MessageID: "msg-new-restart",
		UserID: "b1-restart", UserName: "b1-restart", ChatName: "A group",
		Content: "/new restart recovery", ReplyCtx: "ctx-a",
	})
	opID := NewOperationID("test", sourceKey, "msg-new-restart")
	deadline = time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		op, _ := operations1.Get(opID)
		if op.Status == NewOperationRetryWait && op.Step == NewOperationSessionBound {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	op, _ := operations1.Get(opID)
	if op.Status != NewOperationRetryWait || op.Step != NewOperationSessionBound {
		t.Fatalf("operation before restart = %+v", op)
	}
	if got := len(p1.requests); got != 1 {
		t.Fatalf("new group create calls before restart = %d, want 1", got)
	}
	if err := e1.Stop(); err != nil {
		t.Fatal(err)
	}

	// Process restart: platform Ready automatically finishes the saved work.
	p2 := &reconstructingSpawningPlatform{spawningPlatform: &spawningPlatform{
		stubPlatformEngine: &stubPlatformEngine{n: "test"},
	}}
	agent2 := &cujAgent{}
	e2 := NewEngine("test", agent2, []Platform{p2}, sessionPath, LangEnglish)
	t.Cleanup(func() { _ = e2.Stop() })
	operations2, err := NewNewOperationStore(operationPath)
	if err != nil {
		t.Fatal(err)
	}
	e2.SetNewOperationStore(operations2)
	e2.OnPlatformReady(p2)
	deadline = time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		op, _ = operations2.Get(opID)
		if op.Status == NewOperationStatusCompleted && len(p2.getSent()) >= 2 {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	if op.Status != NewOperationStatusCompleted {
		t.Fatalf("operation after restart = %+v", op)
	}
	if got := len(p2.requests); got != 0 {
		t.Fatalf("new group was created again after restart: calls=%d", got)
	}
	reloadedSource := e2.sessions.GetOrCreateActive(sourceKey)
	reloadedHistory := reloadedSource.GetHistory(0)
	if reloadedSource.ID != sourceID || len(reloadedHistory) != len(sourceHistory) {
		t.Fatalf("source changed across restart: id=%s history=%#v", reloadedSource.ID, reloadedHistory)
	}
	for i := range sourceHistory {
		if reloadedHistory[i].Role != sourceHistory[i].Role || reloadedHistory[i].Content != sourceHistory[i].Content {
			t.Fatalf("source history[%d] changed across restart: got=%#v want=%#v", i, reloadedHistory[i], sourceHistory[i])
		}
	}
	if got := len(e2.sessions.ListSessions(targetKey)); got != 1 {
		t.Fatalf("target sessions after restart = %d, want 1", got)
	}

	// Action 3: B's first real message stays in B and gets a normal reply.
	p2.clearSent()
	e2.ReceiveMessage(p2, &Message{
		SessionKey: targetKey, Platform: "test", MessageID: "msg-target-after-restart",
		UserID: "b1-restart", UserName: "b1-restart", ChatName: "restart recovery",
		Content: "continue in B", ReplyCtx: "ctx-b",
	})
	deadline = time.Now().Add(2 * time.Second)
	for len(p2.getSent()) < 1 && time.Now().Before(deadline) {
		time.Sleep(10 * time.Millisecond)
	}
	if got := strings.Join(p2.getSent(), "\n"); !strings.Contains(got, "ok") {
		t.Fatalf("B did not receive the agent reply: %q", got)
	}
	finalSourceHistory := e2.sessions.GetOrCreateActive(sourceKey).GetHistory(0)
	if len(finalSourceHistory) != len(sourceHistory) || finalSourceHistory[0].Content != sourceHistory[0].Content || finalSourceHistory[1].Content != sourceHistory[1].Content {
		t.Fatal("A history changed after B continued")
	}
}
...
func TestCUJ_H2_TwoPlatformsConcurrentNoBleed(t *testing.T) {
	...
	agent := &cujAgent{}
	e := NewEngine("test", agent, []Platform{pA, pB}, "", LangEnglish)
