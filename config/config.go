...
type HeartbeatConfig struct {
	...
	SessionKey   string `toml:"session_key,omitempty"`    // target session key; required
...
type ProviderConfig struct {
	...
	AgentTypes      []string                         `toml:"agent_types,omitempty"`       // optional: restrict to the "codex" agent type
...
func patchProjectAgentOption(projectName, key, value string) error {
	...
	if projSpan.agentOptionsStart < 0 {
		...
		if projSpan.agentStart < 0 {
			...
			block := []string{"", "[projects.agent]", "type = \"codex\"", "", "[projects.agent.options]"}
			lines = insertLines(lines, insertAt, block)
		} else {
