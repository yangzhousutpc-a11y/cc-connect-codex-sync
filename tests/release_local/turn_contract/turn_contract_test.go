...
func newTurnEngine(t *testing.T) (*core.Engine, *turnAgent, *turnPlatform) {
	...
	engine := core.NewEngine("release-turn", agent, []core.Platform{platform}, "", core.LangEnglish)
	t.Cleanup(func() {
		engine.Stop()
		_ = agent.Stop()
	})
...
func TestStreamingPreviewFinalizationContractExposesDuplicateFinalSend(t *testing.T) {
	...
	engine := core.NewEngine("release-preview", agent, []core.Platform{platform}, "", core.LangEnglish)
	t.Cleanup(func() {
		engine.Stop()
		_ = agent.Stop()
	})
...
func TestStreamingPreviewConfigurationMatrix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			agent := newTurnAgent()
			platform := &previewLifecyclePlatform{}
			engine := core.NewEngine("release-preview-matrix", agent, []core.Platform{platform}, "", core.LangEnglish)
			engine.SetReplyFooterEnabled(true)
...
func TestStreamingPreviewMaxCharsOnlyTruncatesIntermediatePreview(t *testing.T) {
	agent := newTurnAgent()
	platform := &previewLifecyclePlatform{}
	engine := core.NewEngine("release-preview-maxchars", agent, []core.Platform{platform}, "", core.LangEnglish)
...
func TestRichCardModeKeepsToolStepsAndFinalMetadataInOneCard(t *testing.T) {
	...
	engine := core.NewEngine("release-rich-card", agent, []core.Platform{platform}, "", core.LangEnglish)
	engine.SetReplyFooterEnabled(true)
	engine.SetDisplayConfig(core.DisplayCfg{
		Mode:             "full",
		CardMode:         "rich",
